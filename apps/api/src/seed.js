...
async function seed() {
    ...
    await Promise.all([
        ...
        Leave_1.default.deleteMany({}),
        DailyReport_1.default.deleteMany({}),
    ]);
    console.log('🗑️  Cleared existing data');
    // ── Departments ──────────────────────────────────────────────────────────
    const deptDocs = await Department_1.default.insertMany([
    ...
    const hashPw = (pw) => bcryptjs_1.default.hashSync(pw, 12);
    ...
    for (const m of mgrSeed) {
        ...
        mgrUserIds[m.code] = u._id;
    }
    for (const m of mgrSeed) {
        const parts = m.name.split(' ');
        const emp = await Employee_1.default.create({
            employeeCode: m.code,
        ...
        mgrObjIds[m.code] = emp._id;
        await User_1.default.findByIdAndUpdate(mgrUserIds[m.code], { employeeId: emp._id });
        // Set as dept head
        await Department_1.default.findByIdAndUpdate(deptMap[m.dept], { head: emp._id });
    }
    ...
    for (const e of empSeed) {
        ...
        empObjIds[e.code] = emp._id;
        // Link user → employee
        await User_1.default.findByIdAndUpdate(empUserIds[e.code], { employeeId: emp._id });
    }
    ...
    await Attendance_1.default.insertMany(attRecords);
    ...
    // ── Leave Requests ───────────────────────────────────────────────────────
    await Leave_1.default.insertMany([
    ...
    // ── Daily Reports ────────────────────────────────────────────────────────
    await DailyReport_1.default.insertMany([
