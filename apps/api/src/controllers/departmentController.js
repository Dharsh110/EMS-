...
const getAllDepartments = async (_req, res) => {
    try {
        const departments = await Department_1.default.find().populate('head', 'firstName lastName');
        const counts = await Employee_1.default.aggregate([
            { $match: { status: 'active' } },
...
const generateManagerCode = async () => {
    let n = (await Employee_1.default.countDocuments({ employeeCode: /^MGR/ })) + 1;
    let code = `MGR${String(n).padStart(3, '0')}`;
    while (await Employee_1.default.findOne({ employeeCode: code })) {
        n += 1;
...
const promoteToHead = async (dept, employeeId, newEmail) => {
    const employee = await Employee_1.default.findById(employeeId);
    if (!employee)
        throw new Error('Selected department head employee not found.');
    const user = await User_1.default.findById(employee.user);
    if (!user)
        throw new Error('No account linked to the selected employee.');
    let credentials;
    if (user.role !== 'manager') {
        if (newEmail) {
            const emailTaken = await User_1.default.findOne({ email: newEmail.toLowerCase(), _id: { $ne: user._id } });
            if (emailTaken)
    ...
    await Department_1.default.updateMany({ head: employee._id, _id: { $ne: dept._id } }, { $unset: { head: '' } });
    dept.head = employee._id;
    await dept.save();
    return credentials;
};
const createDepartment = async (req, res) => {
    try {
        ...
        const dept = await Department_1.default.create(deptFields);
        let credentials;
        if (head) {
            try {
...
const updateDepartment = async (req, res) => {
    try {
        const { head, headEmail, ...deptFields } = req.body;
        const existing = await Department_1.default.findById(req.params.id);
        if (!existing) {
...
const deleteDepartment = async (req, res) => {
    try {
        const empCount = await Employee_1.default.countDocuments({ department: req.params.id, status: 'active' });
        if (empCount > 0) {
        ...
        const dept = await Department_1.default.findByIdAndDelete(req.params.id);
        if (!dept) {
            res.status(404).json({ success: false, message: 'Department not found.' });
            return;
        }
