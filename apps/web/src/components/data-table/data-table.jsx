...
export function DataTable({ columns, data, isLoading, emptyMessage = 'No results.', pageSize = 10, rowSelection, onRowSelectionChange, showColumnToggle = true, }) {
    ...
    return (<div className="space-y-3">
      {showColumnToggle && (<div className="flex justify-end">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              {table
                ...
                .map((column) => (<DropdownMenuCheckboxItem key={column.id} className="capitalize" checked={column.getIsVisible()} onCheckedChange={(v) => column.toggleVisibility(!!v)}>
                    {column.id}
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (<TableRow>
              ...
              </TableRow>) : table.getRowModel().rows.length ? (table.getRowModel().rows.map((row) => (<TableRow key={row.id} data-state={row.getIsSelected() ? 'selected' : undefined}>
