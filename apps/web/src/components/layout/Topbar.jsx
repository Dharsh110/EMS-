...
export default function Topbar({ title, subtitle, onMenuClick }) {
    ...
    return (<header className="h-14 flex-shrink-0 flex items-center justify-between px-4 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 z-30">
      ...
      <div className="flex items-center gap-1.5">
        {/* Theme toggle */}
        <button onClick={toggleTheme} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-600 dark:text-gray-300 text-xs font-medium hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors">
          {isDark
          ...
          {notifOpen && (<>
              ...
              <div className="absolute right-0 top-full mt-2 w-80 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-2xl z-50 overflow-hidden">
                ...
                <div className="max-h-72 overflow-y-auto divide-y divide-gray-50 dark:divide-gray-700">
                  {unreadNotifs.length === 0 ? (<div className="px-4 py-8 text-center">
                      <p className="text-2xl mb-1">🔔</p>
                      <p className="text-xs text-gray-400">{notifs.length === 0 ? 'No notifications yet' : 'No unread notifications'}</p>
                    </div>) : unreadNotifs.slice(0, 5).map(n => (<div key={n.id} className={`flex gap-3 px-4 py-3 transition-colors ${!n.isRead ? 'bg-blue-50/60 dark:bg-blue-900/10 hover:bg-blue-50 dark:hover:bg-blue-900/15' : 'hover:bg-gray-50 dark:hover:bg-gray-700'}`}>
                      ...
                      <div className="flex flex-col items-center gap-1 flex-shrink-0">
                        {!n.isRead && (<button onClick={() => markRead(n.id)} className="text-[9px] px-1.5 py-0.5 rounded bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 hover:bg-blue-200 font-semibold transition-colors whitespace-nowrap">
                            Read
                          </button>)}
                        <button onClick={() => setDeleteId(n.id)} className="w-5 h-5 flex items-center justify-center rounded hover:bg-red-100 dark:hover:bg-red-900/30 text-gray-400 hover:text-red-500 transition-colors" title="Delete">
        ...
        {/* Profile */}
        <div className="relative">
          <button onClick={() => { setProfileOpen(!profileOpen); setNotifOpen(false); }} className="flex items-center gap-2 pl-1 pr-2 py-1 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors cursor-pointer ml-1">
          ...
          {profileOpen && (<>
              ...
              <div className="absolute right-0 top-full mt-2 w-64 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-2xl z-50 overflow-hidden">
                ...
                <div className="py-1">
                  <button onClick={() => { setProfileOpen(false); navigate(`/${role}/profile`); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-left">
                  ...
                  <button onClick={() => { setProfileOpen(false); navigate(settingsPath); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-left">
      ...
      {deleteId && (<div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
