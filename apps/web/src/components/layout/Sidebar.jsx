...
const Icon = ({ d }) => (<svg className="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const Sidebar = ({ onClose }) => {
    ...
    return (<aside className="sidebar h-full flex flex-col">
      {/* Brand header */}
      <div className="flex items-center gap-3 px-4 py-3.5 border-b border-gray-100 dark:border-gray-700 flex-shrink-0">
        ...
        {onClose && (<button onClick={onClose} className="btn-icon lg:hidden flex-shrink-0">
      ...
      {user?.role === 'manager' && user.department && (<div className="mx-3 mt-2 mb-0.5 px-3 py-2 bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 rounded-xl">
          <div className="flex items-center justify-between gap-2">
            <div className="min-w-0">
              <p className="text-[10px] text-blue-400 dark:text-blue-500 font-medium uppercase tracking-wide">Department</p>
              <p className="text-xs font-semibold text-blue-700 dark:text-blue-300 truncate">{user.department}</p>
            </div>
            {user.managerId && (<span className="flex-shrink-0 text-[9px] font-mono px-2 py-0.5 bg-blue-600 text-white rounded-full">{user.managerId}</span>)}
          </div>
        </div>)}

      <nav className="flex-1 overflow-y-auto px-2 py-2">
        {sections.map(section => (<div key={section.section}>
            <p className="section-title">{section.section}</p>
            {section.items.map(item => (<NavLink key={item.path} to={item.path} onClick={onClose} className={({ isActive }) => clsx(isActive ? 'nav-item-active' : 'nav-item', 'mb-0.5')}>
                {item.icon}
                <span className="flex-1">{item.label}</span>
                {item.badge && (<span className="ml-auto text-xs bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 font-medium px-1.5 py-0.5 rounded-full">
                    {item.badge}
                  </span>)}
              </NavLink>))}
          </div>))}
