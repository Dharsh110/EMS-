...
const fmtDT = (iso) => new Date(iso).toLocaleString('en-IN', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit', hour12: true });
const FILE_ICON = {
...
const isViewable = (type) => type.startsWith('image/') || type === 'application/pdf' || type === 'text/plain' || type === 'text/csv';
const getTypeKey = (type) => TYPE_LABEL[type] || 'Other';
export default function UploadedFiles() {
    ...
    const files = useMemo(() => (apiFiles || []).map((u) => ({
    ...
    const filtered = useMemo(() => {
        ...
        return files.filter((f) => {
            ...
            let matchP = true;
            if (period === 'today')
                matchP = d.toDateString() === n.toDateString();
            else if (period === 'month')
                matchP = d.getFullYear() === selYear && d.getMonth() === selMonth;
            else if (period === 'year')
                matchP = d.getFullYear() === selYear;
    ...
    const getTypeBreakdown = (list) => {
        const map = {};
        list.forEach((f) => { const k = getTypeKey(f.type); if (!map[k])
            map[k] = { count: 0, size: 0 }; map[k].count++; map[k].size += f.size; });
        return Object.entries(map).sort((a, b) => b[1].count - a[1].count);
    };
    // List responses omit `data` (base64) for performance — fetch on demand and cache locally.
    const fetchFileData = async (f) => {
        if (f.base64)
            return f.base64;
        const upload = await fetchUpload(f.id).unwrap();
        if (upload?.data)
            setLocalCache((prev) => ({ ...prev, [f.id]: upload.data }));
        return upload?.data;
    };
    ...
    const openView = async (f) => {
        try {
            ...
            setViewFile({ ...f, base64 });
        }
        catch {
            toast.error('Failed to load file');
        }
    };
    const openSvView = async (f) => {
        try {
            const base64 = await fetchFileData(f);
            if (!base64) {
                toast.error('File data not available');
    ...
    const doDelete = async (targetId = deleteConfirmId) => {
        if (!targetId)
            return;
        try {
            await deleteUploadMut(targetId).unwrap();
    ...
    const openEdit = (f) => { setEditFile(f); setEditName(f.name); setEditNotes(f.notes || ''); };
    const saveEdit = async () => {
        ...
        try {
            await updateUpload({ id: editFile.id, originalName: editName.trim(), notes: editNotes.trim() }).unwrap();
            setEditFile(null);
    ...
    const saveSvEdit = async () => {
        if (!svEdit || !svEditName.trim())
            return;
        try {
            await updateUpload({ id: svEdit.id, originalName: svEditName.trim() }).unwrap();
            setSvEdit(null);
    ...
    const handleUpload = (e) => {
        ...
        if (!picked.length)
            return;
        setUploading(true);
        Promise.all(picked.map((file) => new Promise((resolve, reject) => {
    ...
    if (statView !== 'main') {
        ...
        const svList = baseList.filter((f) => {
            ...
            if (svPeriod === 'year')
                return new Date(f.uploadedAt).getFullYear() === svYear;
            return true;
        });
        const titleMap = { main: '', total: 'Total Files', thismonth: 'This Month', size: 'Total Size' };
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {breakdown.length === 0 ? (<div className="col-span-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-8 text-center"><span className="text-3xl">📁</span><p className="text-sm text-gray-500 mt-2">No files here</p></div>) : breakdown.map(([type, data]) => (<div key={type} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-4">
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
          ...
          {svList.length === 0 ? (<div className="p-8 text-center"><span className="text-3xl">📁</span><p className="text-sm text-gray-500 mt-2">No files</p></div>) : (<div className="divide-y divide-gray-50 dark:divide-gray-700/50">
              {svList.map((f) => (<div key={f.id} className="flex items-center gap-3 px-5 py-3 hover:bg-gray-50 dark:hover:bg-gray-700/30">
        ...
        <Dialog open={!!svView} onOpenChange={(o) => !o && setSvView(null)}>
        ...
        <Dialog open={!!svEdit} onOpenChange={(o) => !o && setSvEdit(null)}>
        ...
        <AlertDialog open={!!svDelId} onOpenChange={(o) => !o && setSvDelId(null)}>
          <AlertDialogContent>
    ...
    return (<div className="space-y-4">
      <div className="grid grid-cols-3 gap-3">
        {[
            { key: 'total', label: 'Total Files', value: filtered.length, icon: '📁', color: 'from-blue-500 to-blue-600' },
            { key: 'thismonth', label: 'This Month', value: filtered.filter((f) => { const d = new Date(f.uploadedAt); return d.getFullYear() === CY && d.getMonth() === new Date().getMonth(); }).length, icon: '📅', color: 'from-purple-500 to-purple-600' },
            { key: 'size', label: 'Total Size', value: fmtBytes(filtered.reduce((s, f) => s + f.size, 0)), icon: '💾', color: 'from-emerald-500 to-emerald-600' },
        ].map((s) => (<div key={s.label} className={`bg-gradient-to-br ${s.color} rounded-2xl p-4 text-white`}>
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl px-4 py-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {['all', 'today', 'month', 'year'].map((p) => (<Button key={p} size="sm" variant={period === p ? 'default' : 'secondary'} className="rounded-lg text-[11px] capitalize" onClick={() => setPeriod(p)}>{p === 'all' ? 'All Time' : p}</Button>))}
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
        ...
        {filtered.length === 0 ? (<div className="px-5 py-14 text-center">
          ...
          </div>) : (<div className="divide-y divide-gray-50 dark:divide-gray-700/50">
            {filtered.map((f) => (<div key={f.id} className="flex items-center gap-4 px-5 py-3.5 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
      ...
      <Dialog open={!!editFile} onOpenChange={(o) => !o && setEditFile(null)}>
        <DialogContent className="max-w-sm">
          ...
          <div className="p-5 space-y-3">
            {editFile && (<div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
...
function TextFileViewer({ base64 }) {
    const [content, setContent] = React.useState('');
    React.useEffect(() => {
        try {
            setContent(atob(base64.split(',')[1]));
        }
        catch {
