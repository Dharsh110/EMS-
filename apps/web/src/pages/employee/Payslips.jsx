...
const printPayslip = (p, name, empId) => {
...
export default function EmployeePayslips() {
    ...
    const payslips = useMemo(() => (apiPayslips || []).map((p) => {
    ...
    return (<div className="space-y-4">
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl px-4 py-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {YEARS.slice().reverse().map((y) => (<Button key={y} size="sm" variant={filterYear === y ? 'default' : 'secondary'} className="rounded-lg text-[11px]" onClick={() => setFilterYear(y)}>{y}</Button>))}
          ...
          <div className="ml-auto flex items-center gap-2">
            <YearDownloadBtn years={YEARS} getRows={(yr) => payslips.filter((p) => p.year === yr)} name={user?.name || 'employee'} onDownload={downloadCSV}/>
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {filtered.length === 0 ? (<TableRow><TableCell colSpan={11} className="px-4 py-12 text-center"><span className="text-3xl">💰</span><p className="text-sm text-gray-500 dark:text-gray-400 mt-2">No payslips for {filterYear}</p></TableCell></TableRow>) : filtered.map((p) => (<TableRow key={p.id}>
      ...
      <Dialog open={!!view} onOpenChange={(o) => !o && setView(null)}>
        <DialogContent className="max-w-md">
          ...
          {view && (<div className="p-5 space-y-3">
              ...
              <div className="space-y-1">
                ...
                {[['Basic Salary', view.basic], ['HRA', view.hra], ['Allowances', view.allowance], ...(view.bonus ? [['Bonus', view.bonus]] : [])].map(([l, v]) => (<div key={l} className="flex justify-between py-1.5 border-b border-gray-50 dark:border-gray-700">
                    <span className="text-xs text-gray-500 dark:text-gray-400">{l}</span>
                    <span className="text-xs font-semibold text-gray-700 dark:text-gray-300">{fmt(v)}</span>
                  </div>))}
              ...
              <div className="space-y-1">
                ...
                {[['Provident Fund', view.pf], ['Income Tax', view.tax]].map(([l, v]) => (<div key={l} className="flex justify-between py-1.5 border-b border-gray-50 dark:border-gray-700">
                    <span className="text-xs text-gray-500 dark:text-gray-400">{l}</span>
...
function YearDownloadBtn({ years, getRows, name, onDownload }) {
    ...
    return (<div className="relative" ref={ref}>
      ...
      {open && (<div className="absolute right-0 top-full mt-1 z-30 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-xl overflow-hidden min-w-[140px]">
          {years.slice().reverse().map((y) => (<button key={y} onClick={() => {
                    const rows = getRows(y);
                    if (!rows.length) {
                        toast.error(`No payslips for ${y}`);
                        return;
                    }
