...
const STATUS_LABEL = { pending: 'Pending', in_progress: 'In Progress', completed: 'Completed', overdue: 'Overdue', cancelled: 'Cancelled' };
const STATUS_ICON = { pending: '⏳', in_progress: '⚡', completed: '✅', overdue: '🚨', cancelled: '❌' };
const P_ICON = { low: '🟢', medium: '🔵', high: '🟠', urgent: '🔴' };
function CircleProgress({ pct, status }) {
    ...
    return (<div className="relative flex-shrink-0 w-14 h-14 flex items-center justify-center">
...
export default function MyTasks() {
    ...
    const dbTasks = useMemo(() => (apiTasks || []).map((t) => ({
        ...
        assignedBy: typeof t.assignedBy === 'object' ? t.assignedBy?.name || 'Manager' : t.assignedBy || 'Manager',
        ...
        return dbTasks.filter((t) => {
            ...
            const inP = period === 'all' ? true
                : period === 'week' ? (dateStr >= weekAgo && dateStr <= todayStr)
                    : period === 'month' ? ((d) => d.getMonth() === selMonth && d.getFullYear() === selYear)(new Date(dateStr))
    ...
    const isOverdue = (t) => t.due < todayStr && t.status !== 'completed' && t.status !== 'cancelled';
    return (<div className="space-y-4">
      <div className="grid grid-cols-4 gap-2 sm:grid-cols-8">
        {summaryCards.map((s) => (<button key={s.key} onClick={() => setStatusF((p) => (p === s.key ? '' : s.key))} className={`${s.bg} border rounded-2xl p-3 text-center transition-all hover:scale-105 ${statusF === s.key ? 'ring-2 ring-blue-500 scale-105' : ''}`}>
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl px-4 py-3">
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex flex-wrap gap-1">
            {['all', 'week', 'month', 'year'].map((v) => (<Button key={v} size="sm" variant={period === v ? 'default' : 'secondary'} className="rounded-lg text-[11px]" onClick={() => setPeriod(v)}>
                {v === 'all' ? 'All' : v === 'week' ? 'This Week' : v === 'month' ? 'Month' : 'Year'}
              </Button>))}
          </div>
          {period === 'month' && (<Select value={String(selMonth)} onValueChange={(v) => setSelMonth(+v)}>
              <SelectTrigger className="w-24 h-8 text-xs"><SelectValue /></SelectTrigger>
              <SelectContent>{MOS.map((m, i) => <SelectItem key={m} value={String(i)}>{m}</SelectItem>)}</SelectContent>
            </Select>)}
          {(period === 'month' || period === 'year') && (<Select value={String(selYear)} onValueChange={(v) => setSelYear(+v)}>
          ...
          <Select value={statusF || 'all'} onValueChange={(v) => setStatusF(v === 'all' ? '' : v)}>
            ...
            <SelectContent>
              ...
              {['pending', 'in_progress', 'completed', 'overdue', 'cancelled'].map((s) => <SelectItem key={s} value={s}>{STATUS_LABEL[s]}</SelectItem>)}
            </SelectContent>
          </Select>
      ...
      {isLoading ? (<div className="flex items-center justify-center py-16">
        ...
        </div>) : filtered.length === 0 ? (<div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl px-4 py-16 text-center">
          ...
          {filtered.map((t) => {
                ...
                return (<div key={t.id} className={`bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 border-l-4 ${CARD_ACCENT[t.status]} rounded-2xl overflow-hidden hover:shadow-md transition-all`}>
                ...
                <button onClick={() => setExpand(expand === t.id ? null : t.id)} className="w-full px-4 pb-3 flex items-center gap-1 text-[10px] text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors text-left">
                  <span>{expand === t.id ? '▴ Hide' : '▾ Show'} description</span>
                </button>
                {expand === t.id && (<div className="px-4 pb-4 pt-0 border-t border-gray-50 dark:border-gray-700/50">
