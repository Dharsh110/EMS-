...
export default function MyLeaves() {
    ...
    const leaves = useMemo(() => (apiLeaves || []).map((l) => ({
        ...
        days: l.totalDays, reason: l.reason, applied: l.appliedAt?.slice(0, 10) || '', status: l.status,
    })), [apiLeaves]);
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl px-4 py-3">
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex flex-wrap gap-1">
            {['all', 'pending', 'approved', 'rejected', 'cancelled'].map((s) => (<Button key={s} size="sm" variant={statusF === s ? 'default' : 'secondary'} className="rounded-lg text-[11px] capitalize" onClick={() => setStatusF(s)}>{s} ({counts[s]})</Button>))}
          </div>
          <Select value={selMonth === '' ? 'all' : String(selMonth)} onValueChange={(v) => setSelMonth(v === 'all' ? '' : +v)}>
          ...
          <Select value={selYear === '' ? 'all' : String(selYear)} onValueChange={(v) => setSelYear(v === 'all' ? '' : +v)}>
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {filtered.length === 0 ? (<TableRow><TableCell colSpan={8} className="px-4 py-12 text-center">
                  <div className="flex flex-col items-center gap-2">
                    <span className="text-3xl">🗓️</span>
                    <p className="text-sm text-gray-500 dark:text-gray-400">No leave records found</p>
                  </div>
                </TableCell></TableRow>) : filtered.map((l) => (<TableRow key={l.id}>
                  ...
                  <TableCell>
                    {l.status === 'pending' ? (<button onClick={() => cancel(l.id)} title="Cancel application" className="w-6 h-6 flex items-center justify-center bg-gray-100 dark:bg-gray-700 text-gray-500 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/20 hover:text-red-500 font-bold text-sm transition-colors">✕</button>) : <span className="text-[10px] text-gray-400">—</span>}
                  </TableCell>
                </TableRow>))}
            </TableBody>
          </Table>
      ...
      <Dialog open={modal} onOpenChange={setModal}>
        <DialogContent className="max-w-md">
          ...
          <div className="p-5 space-y-3">
            ...
            {daysCalc > 0 && (<div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl px-4 py-2.5 flex items-center justify-between">
