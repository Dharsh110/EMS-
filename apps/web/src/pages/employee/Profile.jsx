...
const EMPTY_PROFILE = { name: '', email: '', phone: '', designation: '', dept: '', empId: '', joined: '', gender: '', dob: '', address: '', blood: '', emergency: { name: '', relation: '', phone: '' }, skills: [], bio: '' };
export default function EmployeeProfile() {
    ...
    const [profile, setProfile] = useState({ ...EMPTY_PROFILE, name: user?.name || '', email: user?.email || '' });
    const [form, setForm] = useState({ ...EMPTY_PROFILE, name: user?.name || '', email: user?.email || '' });
    useEffect(() => {
        const e = apiProfile;
        if (!e)
            return;
        const merged = {
            ...
            address: typeof e.address === 'object' && e.address ? [e.address.street, e.address.city, e.address.state, e.address.pincode].filter(Boolean).join(', ') : e.address || '',
            blood: e.bloodGroup || '',
    ...
    const handleAvatar = (e) => {
        ...
        reader.onload = async (ev) => {
            ...
            setAvatar(base64);
            try {
                await updateMyProfile({ avatar: base64 }).unwrap();
    ...
    const FLD = ({ label, k, type = 'text', disabled = false }) => (<div>
      <Label className="mb-1 block">{label}</Label>
      {edit && !disabled ? (<Input type={type} value={form[k] || ''} onChange={(e) => setForm((p) => ({ ...p, [k]: e.target.value }))}/>) : (<p className="text-sm font-medium text-gray-800 dark:text-gray-200 py-2 px-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl">{profile[k] || '—'}</p>)}
    </div>);
    return (<div className="max-w-2xl mx-auto space-y-5">
        ...
        <div className="px-5 pb-5">
          <div className="flex items-end justify-between -mt-12 mb-4 flex-wrap gap-3">
            <div className="relative">
              {avatar ? (<img src={avatar} alt="avatar" className="w-20 h-20 rounded-2xl object-cover border-4 border-white dark:border-gray-800 shadow-lg"/>) : (<div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-blue-500 to-blue-700 flex items-center justify-center text-white text-2xl font-bold border-4 border-white dark:border-gray-800 shadow-lg">{initials}</div>)}
              {edit && (<button onClick={() => fileRef.current?.click()} className="absolute -bottom-1 -right-1 w-7 h-7 bg-blue-600 hover:bg-blue-700 text-white rounded-full flex items-center justify-center text-sm shadow transition-colors" title="Change photo">📷</button>)}
              <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={handleAvatar}/>
            </div>
            <div className="flex gap-2 mt-8">
              {!edit ? (<Button variant="outline" size="sm" className="rounded-xl" onClick={() => setEdit(true)}>✏️ Edit Profile</Button>) : (<>
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-5 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            {edit ? (<Select value={form.gender || undefined} onValueChange={(v) => setForm((p) => ({ ...p, gender: v }))}>
              ...
              </Select>) : (<p className="text-sm font-medium text-gray-800 dark:text-gray-200 py-2 px-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl">{profile.gender || '—'}</p>)}
          </div>
          <FLD label="Blood Group" k="blood"/>
          <div className="col-span-2"><FLD label="Address" k="address"/></div>
          {edit && (<div className="col-span-2">
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-5 space-y-3">
        ...
        <div className="grid grid-cols-3 gap-3">
          {['name', 'relation', 'phone'].map((k) => (<div key={k}>
              <Label className="mb-1 block capitalize">{k}</Label>
              {edit ? (<Input value={form.emergency[k] || ''} onChange={(e) => setForm((p) => ({ ...p, emergency: { ...p.emergency, [k]: e.target.value } }))}/>) : (<p className="text-sm font-medium text-gray-800 dark:text-gray-200 py-2 px-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl">{profile.emergency[k] || '—'}</p>)}
            </div>))}
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-5">
        ...
        {edit ? (<div className="flex flex-wrap gap-2">
            {SKILLS_LIST.map((s) => (<button key={s} onClick={() => toggleSkill(s)} className={`text-xs px-3 py-1.5 rounded-xl border font-medium transition-all ${form.skills.includes(s) ? 'bg-blue-600 text-white border-blue-600' : 'border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-400 hover:border-blue-400 dark:hover:border-blue-500'}`}>
