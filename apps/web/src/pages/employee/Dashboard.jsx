...
const LEAVE_LIMITS = { casual: 10, sick: 6, earned: 15 };
const LEAVE_LABELS = { casual: 'Casual', sick: 'Sick', earned: 'Earned' };
const LEAVE_COLORS = { casual: 'bg-blue-500', sick: 'bg-red-500', earned: 'bg-emerald-500' };
export default function EmployeeDashboard() {
    ...
    return (<div className="space-y-5">
      ...
      <div className="bg-gradient-to-r from-blue-600 via-blue-700 to-indigo-700 rounded-2xl p-5 text-white relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-start justify-between flex-wrap gap-4">
          <div>
            ...
            {checkedIn && (<div className="flex items-center gap-3 mt-3 flex-wrap">
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-5">
        ...
        {!checkedIn && late && (<div className="flex items-center gap-2 mb-3 px-3 py-2 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl">
        ...
        {!checkedIn && (<div className="mb-3">
            ...
            {showNotes && (<Textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={2} placeholder="e.g. Working from home…" className="mt-2"/>)}
          </div>)}
        <div className="flex items-center gap-3 flex-wrap">
          {!checkedIn ? (<Button onClick={doCheckIn} disabled={attLoading} size="lg" className={`rounded-xl font-bold shadow-lg hover:scale-105 transition-all ${late ? 'bg-amber-500 hover:bg-amber-600' : 'bg-emerald-500 hover:bg-emerald-600'}`}>
              🟢 {attLoading ? 'Checking in…' : `Check In ${late ? '(Late)' : ''}`}
            </Button>) : !checkedOut ? (<Button onClick={doCheckOut} disabled={attLoading} size="lg" variant="destructive" className="rounded-xl font-bold shadow-lg hover:scale-105 transition-all">
              🔴 {attLoading ? 'Checking out…' : 'Check Out'}
            </Button>) : (<div className="flex items-center gap-2 px-4 py-2.5 bg-gray-100 dark:bg-gray-700 rounded-xl">
              <span>✅</span>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {quickStats.map((s) => (<button key={s.label} onClick={() => navigate(s.path)} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-3.5 text-left hover:shadow-md hover:-translate-y-0.5 transition-all">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
          ...
          {recentTasks.length === 0 ? (<div className="px-5 py-8 text-center">
              ...
              {recentTasks.map((t) => (<div key={t._id} className="px-5 py-3">
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
        ...
        ].map((a) => (<button key={a.label} onClick={() => navigate(a.path)} className={`flex items-center gap-2 px-3 py-2.5 rounded-xl text-xs font-semibold transition-colors ${a.color}`}>
