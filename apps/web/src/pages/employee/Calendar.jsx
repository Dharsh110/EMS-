...
const BLANK_EVENT = { title: '', type: 'meeting', date: '', endDate: '', description: '' };
export default function EmployeeCalendar() {
    ...
    const userEvents = (apiEvents || []).map((e) => ({
        id: e._id, title: e.title, type: e.type || 'meeting',
    ...
    const dayEvents = (day) => userEvents.filter((e) => { const d = new Date(e.date); return d.getFullYear() === calYear && d.getMonth() === calMonth && d.getDate() === day; });
    ...
    return (<div className="space-y-5">
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-700 flex-wrap gap-2">
          <div className="flex items-center gap-3">
            ...
            <Select value={String(calMonth)} onValueChange={(v) => setCalMonth(+v)}>
            ...
            <Select value={String(calYear)} onValueChange={(v) => setCalYear(+v)}>
        ...
        <div className="p-4">
          ...
          <div className="grid grid-cols-7 gap-1">
            {cells.map((day, idx) => {
            if (!day)
                return <div key={`e-${idx}`} className="h-14 sm:h-16"/>;
            const de = dayEvents(day);
            const isTd = isToday(day);
            return (<div key={day} className={`h-14 sm:h-16 rounded-xl p-1 border ${isTd ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-100 dark:border-gray-700'} hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors`}>
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
        ...
        <div className="divide-y divide-gray-50 dark:divide-gray-700/50">
          {monthEvents.length === 0 ? (<div className="px-4 py-12 text-center"><span className="text-3xl">📅</span><p className="text-sm text-gray-500 dark:text-gray-400 mt-2">No events in {MONTHS_FULL[calMonth]} {calYear}</p></div>) : monthEvents.map((e) => (<div key={e.id} className="flex items-start gap-4 px-5 py-3.5 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
          ...
          <div className="p-5 space-y-3">
            ...
            <div>
              ...
              <Select value={newEvent.type} onValueChange={(v) => setNewEvent((p) => ({ ...p, type: v }))}>
                ...
                <SelectContent>{['meeting', 'reminder', 'deadline', 'company_event', 'holiday', 'birthday'].map((t) => <SelectItem key={t} value={t} className="capitalize">{ET_ICON[t]} {t.replace('_', ' ')}</SelectItem>)}</SelectContent>
              </Select>
            </div>
