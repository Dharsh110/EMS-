...
const PWD_CHECKS = [
    { label: '8+ chars', fn: (v) => v.length >= 8 },
    { label: 'Uppercase', fn: (v) => /[A-Z]/.test(v) },
    { label: 'Number', fn: (v) => /[0-9]/.test(v) },
    { label: 'Symbol', fn: (v) => /[^A-Za-z0-9]/.test(v) },
];
const S_CLR = ['', 'bg-red-500', 'bg-orange-400', 'bg-yellow-400', 'bg-emerald-500'];
const S_LBL = ['', 'Weak', 'Fair', 'Good', 'Strong'];
const Toggle = ({ label, k, desc, state, setState }) => (<div className="flex items-center justify-between py-3 border-b border-gray-50 dark:border-gray-700 last:border-0">
...
export default function EmployeeSettings() {
    ...
    useEffect(() => {
        ...
        if (d.preferences?.notifications)
            setNotifs((n) => ({ ...n, ...d.preferences.notifications }));
        if (d.preferences?.privacy)
            setPrivacy((p) => ({ ...p, ...d.preferences.privacy }));
        if (d.preferences?.ui)
            setPrefs((p) => ({ ...p, ...d.preferences.ui }));
    }, [apiProfile]);
    const [ticket, setTicket] = useState({ subject: '', message: '', priority: 'medium' });
    const [ticketSent, setTicketSent] = useState(false);
    const saveAccount = async () => {
        try {
            await updateMyProfile({ phone: account.phone, preferences: { timezone: account.timezone, language: account.language } }).unwrap();
    ...
    const saveNotifs = async () => { try {
        await updateMyProfile({ preferences: { notifications: notifs } }).unwrap();
        toast.success('Notification preferences saved!');
    }
    catch {
        toast.error('Failed to save notification preferences');
    } };
    const savePrivacy = async () => { try {
        await updateMyProfile({ preferences: { privacy } }).unwrap();
        toast.success('Privacy settings saved!');
    }
    catch {
        toast.error('Failed to save privacy settings');
    } };
    const savePrefs = async () => { try {
        await updateMyProfile({ preferences: { ui: prefs } }).unwrap();
    ...
    return (<div className="max-w-2xl mx-auto space-y-5">
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-1 flex gap-1 flex-wrap">
        {TABS.map((t) => (<button key={t.id} onClick={() => setTab(t.id)} className={`flex-1 flex items-center justify-center gap-1.5 py-2 text-xs font-semibold rounded-xl transition-all whitespace-nowrap ${tab === t.id ? 'bg-blue-600 text-white shadow' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700'}`}>
      ...
      {tab === 'account' && (<div className="space-y-4">
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-5 space-y-4">
            ...
            {[['Full Name', 'name', 'text'], ['Email Address', 'email', 'email'], ['Phone Number', 'phone', 'tel']].map(([l, k, t]) => (<div key={k}>
                <Label className="mb-1 block">{l}</Label>
                <Input type={t} value={account[k] || ''} disabled={k === 'email'} onChange={(e) => setAccount((p) => ({ ...p, [k]: e.target.value }))}/>
              </div>))}
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label className="mb-1 block">Timezone</Label>
                <Select value={account.timezone} onValueChange={(v) => setAccount((p) => ({ ...p, timezone: v }))}>
              ...
              <div>
                ...
                <Select value={account.language} onValueChange={(v) => setAccount((p) => ({ ...p, language: v }))}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
          ...
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-5 space-y-4">
            ...
            {[['Current Password', 'current', 'c'], ['New Password', 'next', 'n'], ['Confirm New Password', 'confirm', 'cf']].map(([l, k, sk]) => (<div key={k}>
                <Label className="mb-1 block">{l}</Label>
                <div className="relative">
                  <Input type={showP[sk] ? 'text' : 'password'} value={pwd[k]} className="pr-14" onChange={(e) => setPwd((p) => ({ ...p, [k]: e.target.value }))}/>
                ...
                {k === 'next' && pwd.next && (<div className="mt-2">
      ...
      {tab === 'notifications' && (<div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-5">
      ...
      {tab === 'privacy' && (<div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-5">
      ...
      {tab === 'preferences' && (<div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-5">
          ...
          <div className="mt-4 pt-4 border-t border-gray-50 dark:border-gray-700">
            ...
            <div className="grid grid-cols-2 gap-2">
              {[['Employee ID', empInfo.empId], ['Department', empInfo.dept], ['Designation', empInfo.designation], ['Joining Date', empInfo.joined], ['Work Location', empInfo.workLocation]].map(([l, v]) => (<div key={l} className="bg-gray-50 dark:bg-gray-700/50 rounded-xl px-3 py-2">
      ...
      {tab === 'help' && (<div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            ].map((c) => (<div key={c.title} className={`bg-gradient-to-br ${c.color} rounded-2xl p-4 text-white`}>
          ...
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-5">
            ...
            <div className="space-y-1">
              {FAQ.map((f) => (<details key={f.q} className="group rounded-xl border border-gray-100 dark:border-gray-700 overflow-hidden">
          ...
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-5">
            ...
            {ticketSent ? (<div className="py-6 text-center">
                ...
                <div>
                  ...
                  <Select value={ticket.priority} onValueChange={(v) => setTicket((p) => ({ ...p, priority: v }))}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
