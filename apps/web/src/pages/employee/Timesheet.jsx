...
import { useGetMyTimesheetsQuery, useSaveTimesheetDraftMutation, useSubmitTimesheetMutation, useResubmitTimesheetMutation, } from '../../store/api/timesheetsApi';
...
const blankEntry = () => ({ key: Date.now().toString() + Math.random().toString(36).slice(2), task: '', description: '', timeSpentMinutes: 60, remarks: '' });
const fmtMinutes = (m) => `${Math.floor(m / 60)}h ${m % 60 ? `${m % 60}m` : ''}`.trim();
export default function EmployeeTimesheet() {
    ...
    const displayEntries = entries ?? (current?.entries.map((e) => ({ ...e, key: e.task + Math.random() })) ?? [blankEntry()]);
    ...
    const removeEntry = (key) => setEntries(displayEntries.length > 1 ? displayEntries.filter((e) => e.key !== key) : displayEntries);
    ...
    const doSaveDraft = async () => {
        ...
        if (!valid.length) {
            toast.error('Add at least one task with a task name and time spent');
            return;
        }
        try {
    ...
    const doSubmit = async () => {
        ...
        try {
            ...
            toast.error(err?.data?.message || 'Failed to submit timesheet');
        }
    };
    const doResubmit = async () => {
        ...
        try {
            ...
            toast.error(err?.data?.message || 'Failed to resubmit timesheet');
        }
    };
    const [histFrom, setHistFrom] = useState('');
    const [histTo, setHistTo] = useState('');
    const history = useMemo(() => [...timesheets].sort((a, b) => b.date.localeCompare(a.date)), [timesheets]);
    const filteredHistory = useMemo(() => history.filter((t) => (!histFrom || t.date.slice(0, 10) >= histFrom) && (!histTo || t.date.slice(0, 10) <= histTo)), [history, histFrom, histTo]);
    ...
    return (<div className="space-y-5">
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
        ...
        {current?.status === 'rejected' && current.rejectionReason && (<div className="mx-5 mt-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-3">
        ...
        <div className="p-5 space-y-3">
          {displayEntries.map((entry, i) => (<div key={entry.key} className="grid grid-cols-1 sm:grid-cols-[1.5fr_2fr_100px_1.5fr_auto] gap-2 items-start bg-gray-50/50 dark:bg-gray-700/20 border border-gray-100 dark:border-gray-700 rounded-xl p-3">
              <div>
              ...
              {isEditable && (<button type="button" onClick={() => removeEntry(entry.key)} className="mt-5 text-red-400 hover:text-red-600 text-xs font-bold h-8" title="Remove entry">✕</button>)}
            </div>))}

          {isEditable && (<Button type="button" variant="outline" size="sm" className="text-xs" onClick={addEntry}>+ Add Task Entry</Button>)}
          ...
          <div className="flex items-center justify-between pt-2 border-t border-gray-100 dark:border-gray-700">
            ...
            {isEditable && (<div className="flex items-center gap-2">
                ...
                {current?.status === 'rejected' ? (<Button size="sm" disabled={resubmitting} onClick={doResubmit}>{resubmitting ? 'Resubmitting…' : 'Resubmit for Approval'}</Button>) : (<Button size="sm" disabled={submitting} onClick={doSubmit}>{submitting ? 'Submitting…' : 'Submit for Approval'}</Button>)}
              </div>)}
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
        ...
        {isLoading ? (<div className="px-5 py-10 text-center text-sm text-gray-400">Loading…</div>) : filteredHistory.length === 0 ? (<div className="px-5 py-10 text-center"><span className="text-3xl">🗓️</span><p className="text-sm text-gray-500 dark:text-gray-400 mt-2">{history.length === 0 ? 'No timesheets yet' : 'No timesheets match this date range'}</p></div>) : (<div className="divide-y divide-gray-50 dark:divide-gray-700/50">
            {filteredHistory.map((t) => (<button key={t._id} onClick={() => setViewHistory(t)} className="w-full text-left px-5 py-3 hover:bg-gray-50 dark:hover:bg-gray-700/20 transition-colors flex items-center justify-between gap-3">
                <div>
      ...
      <Dialog open={!!viewHistory} onOpenChange={(o) => !o && setViewHistory(null)}>
        <DialogContent className="max-w-lg">
          ...
          {viewHistory && (<DialogBody>
              <div className="space-y-2">
                {viewHistory.entries.map((e, i) => (<div key={i} className="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-3 space-y-1.5">
              ...
              {viewHistory.rejectionReason && (<div className="mt-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-3">
              ...
              <div className="mt-3">
                ...
                <div className="space-y-1">
                  {viewHistory.auditTrail.map((a, i) => (<p key={i} className="text-[10px] text-gray-400">
          ...
          <DialogFooter>
            {(viewHistory?.status === 'draft' || viewHistory?.status === 'rejected') && (<Button variant="outline" className="flex-1" onClick={() => { loadForDate(viewHistory.date.slice(0, 10)); setViewHistory(null); }}>Edit</Button>)}
