...
const dlPDF = (rows, headers, title) => {
...
const deptOfEmp = (e) => (e ? (typeof e.department === 'object' ? e.department?.name : e.department) || '—' : '—');
export default function ManagerReports() {
    ...
    const attRows = useMemo(() => apiAttendance.map((r) => {
    ...
    const leaveRows = useMemo(() => apiLeaves.map((r) => {
        const emp = typeof r.employee === 'object' ? r.employee : null;
    ...
    const taskDept = (r) => {
        if (typeof r.department === 'object' && r.department)
            return r.department.name;
        if (typeof r.department === 'string' && r.department)
            return r.department;
        const a = typeof r.assignedTo === 'object' ? r.assignedTo : null;
        return deptOfEmp(a);
    };
    const taskRows = useMemo(() => apiTasks.map((r) => {
        const a = typeof r.assignedTo === 'object' ? r.assignedTo : null;
    ...
    const assignRows = useMemo(() => apiTasks.map((r) => {
        const a = typeof r.assignedTo === 'object' ? r.assignedTo : null;
    ...
    const doCSV = () => {
        ...
        if (tab === 'attendance')
            dlCSV([['Report Date', 'Date', 'Emp Code', 'Employee', 'Check-In', 'Check-Out', 'Hours', 'Status'], ...filtAtt.map((r) => [genDate, r.date, r.code, r.name, r.checkIn, r.checkOut, r.hours > 0 ? r.hours.toFixed(2) + 'h' : '0h', r.status])], 'attendance_report.csv');
        if (tab === 'leave')
            dlCSV([['Report Date', 'Applied On', 'Employee', 'Type', 'From', 'To', 'Days', 'Status'], ...filtLeave.map((r) => [genDate, r.date, r.name, r.type, r.from, r.to, String(r.days) + 'd', r.status])], 'leave_report.csv');
        if (tab === 'task')
            dlCSV([['Report Date', 'Date', 'Employee', 'Task', 'Status', 'Progress', 'Due'], ...filtTask.map((r) => [genDate, r.date, r.name, r.title, r.status.replace(/_/g, ' '), r.progress + '%', r.due])], 'task_report.csv');
        if (tab === 'assigned')
            dlCSV([['Report Date', 'Assigned On', 'Employee', 'Task', 'Priority', 'Due'], ...filtAssign.map((r) => [genDate, r.assignedOn, r.name, r.title, r.priority, r.due])], 'assigned_tasks.csv');
        if (tab === 'members')
            dlCSV([['Report Date', 'Code', 'Name', 'Department', 'Designation', 'Joining Date', 'Status'], ...members.map((m) => [genDate, m.code, m.name, m.dept, m.designation, m.joining, m.status])], 'team_members.csv');
        toast.success('CSV downloaded');
    };
    const doPDF = () => {
        if (tab === 'attendance')
            dlPDF(filtAtt.map((r) => [r.date, r.name, r.checkIn, r.checkOut, r.hours > 0 ? r.hours.toFixed(2) + 'h' : '—', r.status]), ['Date', 'Employee', 'Check In', 'Check Out', 'Hours', 'Status'], 'Team Attendance Report');
        if (tab === 'leave')
            dlPDF(filtLeave.map((r) => [r.date, r.name, r.type, r.from, r.to, r.days + 'd', r.status]), ['Applied On', 'Employee', 'Type', 'From', 'To', 'Days', 'Status'], 'Leave Report');
        if (tab === 'task')
            dlPDF(filtTask.map((r) => [r.date, r.name, r.title, r.status.replace('_', ' '), r.progress + '%', r.due]), ['Date', 'Employee', 'Task', 'Status', 'Progress', 'Due'], 'Task Report');
        if (tab === 'assigned')
            dlPDF(filtAssign.map((r) => [r.assignedOn, r.name, r.title, r.priority, r.due]), ['Assigned On', 'Employee', 'Task', 'Priority', 'Due Date'], 'Assigned Tasks');
        if (tab === 'members')
            dlPDF(members.map((m) => [m.code, m.name, m.dept, m.designation, m.joining, m.status]), ['Code', 'Name', 'Department', 'Designation', 'Joining Date', 'Status'], 'Team Members');
        toast.success('PDF report opening…');
    };
    const TABS = [{ v: 'attendance', l: 'Attendance' }, { v: 'leave', l: 'Leave' }, { v: 'task', l: 'Tasks' }, { v: 'assigned', l: 'Assigned' }, { v: 'members', l: 'Team Members' }];
    const SortBtn = ({ col, label }) => (<button onClick={() => toggleSort(col)} className="flex items-center gap-0.5 hover:text-blue-500 whitespace-nowrap">{label}<span className="text-[9px] opacity-60 ml-0.5">{sort.col === col ? (sort.dir === 'asc' ? '↑' : '↓') : '↕'}</span></button>);
    const TH = ({ col, label }) => <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide"><SortBtn col={col} label={label}/></th>;
    ...
    const EmptyRow = ({ cols }) => <tr><td colSpan={cols} className="py-10 text-center"><p className="text-sm text-gray-400">No data found</p></td></tr>;
    return (<div className="space-y-4">
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl px-4 py-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex flex-wrap items-center gap-1">
            {['all', 'today', 'week', 'month', 'year'].map((p) => <Button key={p} size="sm" variant={period === p ? 'default' : 'secondary'} className="capitalize" onClick={() => setPeriod(p)}>{p}</Button>)}
            {period === 'month' && <Select value={String(selMonth)} onValueChange={(v) => setSelMonth(Number(v))}><SelectTrigger className="h-8 w-24 text-xs"><SelectValue /></SelectTrigger><SelectContent>{MONTHS.map((m, i) => <SelectItem key={m} value={String(i)}>{m}</SelectItem>)}</SelectContent></Select>}
            {(period === 'month' || period === 'year') && <Select value={String(selYear)} onValueChange={(v) => setSelYear(Number(v))}><SelectTrigger className="h-8 w-24 text-xs"><SelectValue /></SelectTrigger><SelectContent>{YEARS.map((y) => <SelectItem key={y} value={String(y)}>{y}</SelectItem>)}</SelectContent></Select>}
            {!myDept && (<Select value={deptF || 'all'} onValueChange={(v) => setDeptF(v === 'all' ? '' : v)}>
      ...
      {tab === 'attendance' && (<div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
          <div className="overflow-x-auto"><table className="w-full">
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-gray-700/50">
              {filtAtt.length === 0 ? <EmptyRow cols={7}/> : filtAtt.map((r, i) => (<tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-700/30">
      ...
      {tab === 'leave' && (<div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
          <div className="overflow-x-auto"><table className="w-full">
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-gray-700/50">
              {filtLeave.length === 0 ? <EmptyRow cols={7}/> : filtLeave.map((r, i) => (<tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                  <td className="px-3 py-2.5 text-xs text-gray-600 dark:text-gray-400">{r.date}</td>
                  <td className="px-3 py-2.5 text-xs font-medium text-gray-800 dark:text-gray-200">{r.name}</td>
      ...
      {tab === 'task' && (<div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
          <div className="overflow-x-auto"><table className="w-full">
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-gray-700/50">
              {filtTask.length === 0 ? <EmptyRow cols={6}/> : filtTask.map((r, i) => (<tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                  <td className="px-3 py-2.5 text-xs text-gray-600 dark:text-gray-400">{r.date}</td>
                  <td className="px-3 py-2.5 text-xs font-medium text-gray-800 dark:text-gray-200">{r.name}</td>
      ...
      {tab === 'assigned' && (<div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
          <div className="overflow-x-auto"><table className="w-full">
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-gray-700/50">
              {filtAssign.length === 0 ? <EmptyRow cols={5}/> : filtAssign.map((r, i) => (<tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-700/30">
      ...
      {tab === 'members' && (<div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
          ...
          <div className="overflow-x-auto"><table className="w-full">
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-gray-700/50">
              {members.length === 0 ? <EmptyRow cols={6}/> : sorted(members).map((m, i) => (<tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-700/30">
