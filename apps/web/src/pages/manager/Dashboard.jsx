...
const P_CLR = { urgent: 'bg-red-100 text-red-700', high: 'bg-amber-100 text-amber-700', medium: 'bg-blue-100 text-blue-700', low: 'bg-gray-100 text-gray-600' };
...
export default function ManagerDashboard() {
    ...
    const teamAtt = useMemo(() => (todayData?.records ?? []).filter((a) => {
    ...
    const teamRoster = useMemo(() => apiEmployees.filter((e) => e.status === 'active' && (!mgrDept || (typeof e.department === 'object' ? e.department?.name : e.department) === mgrDept)), [apiEmployees, mgrDept]);
    ...
    const approveLeave = async (id) => { try {
    ...
    const rejectLeave = async (id) => { try {
        await updateLeaveStatus({ id, status: 'rejected', reason: 'Rejected by manager' }).unwrap();
    ...
    const empName = (a) => (typeof a.employee === 'object' ? `${a.employee.firstName} ${a.employee.lastName}` : 'Employee');
    const empCode = (a) => (typeof a.employee === 'object' ? a.employee.employeeCode || '' : '');
    const assigneeName = (t) => (typeof t.assignedTo === 'object' ? `${t.assignedTo.firstName} ${t.assignedTo.lastName}` : 'Unassigned');
    const statCards = [
    ...
    return (<div className="space-y-5">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center gap-4 flex-wrap">
            {myAtt ? (<>
                ...
                {myAtt.checkOut ? (<div className="text-center"><p className="text-[10px] text-gray-400 uppercase tracking-wide">Checked Out</p><p className="text-sm font-bold text-blue-600">{fmtTime(myAtt.checkOut)}</p></div>) : (<Button variant="secondary" className="bg-red-500 hover:bg-red-600 text-white" onClick={doCheckOut} disabled={ciLoading}>{ciLoading ? 'Checking out…' : 'Check Out'}</Button>)}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {statCards.map((s) => (<button key={s.label} onClick={() => setModal(s.key)} className={`border rounded-2xl p-4 text-left hover:scale-105 transition-all ${s.bg}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
          ...
          <div className="divide-y divide-gray-50 dark:divide-gray-700/50">
            {pendingL.length === 0 ? (<div className="px-5 py-8 text-center"><span className="text-2xl">✅</span><p className="text-xs text-gray-500 dark:text-gray-400 mt-2">No pending leave requests</p></div>) : pendingL.slice(0, 4).map((l, i) => (<div key={l.id} className="flex items-center justify-between px-5 py-3">
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
          ...
          <div className="divide-y divide-gray-50 dark:divide-gray-700/50">
            {activeTasks.length === 0 ? (<div className="px-5 py-8 text-center"><span className="text-2xl">📋</span><p className="text-xs text-gray-500 dark:text-gray-400 mt-2">No active tasks for {mgrDept || 'your team'}</p></div>) : activeTasks.slice(0, 5).map((t) => (<div key={t._id} className="flex items-start gap-3 px-5 py-3">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-gray-700/50">
              {teamAtt.length === 0 ? (<tr><td colSpan={5} className="px-4 py-10 text-center text-sm text-gray-400">{mgrDept ? `No attendance data for ${mgrDept} today` : 'No attendance data for today'}</td></tr>) : displayTeam.map((a, i) => (<tr key={a._id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-4">
        ...
        ].map((a) => (<button key={a.label} onClick={() => navigate(a.path)} className={`flex items-center gap-2 px-3 py-2.5 rounded-xl text-xs font-semibold transition-colors ${a.cls}`}><span>{a.icon}</span>{a.label}</button>))}
        </div>
      </div>

      <Dialog open={modal === 'present'} onOpenChange={(o) => !o && setModal(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader><DialogTitle>✅ Present Today ({present.length})</DialogTitle></DialogHeader>
          <DialogBody>
            {present.map((a, i) => (<div key={a._id} className="flex items-center gap-3 py-2">
      ...
      <Dialog open={modal === 'absent'} onOpenChange={(o) => !o && setModal(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader><DialogTitle>❌ Absent Today ({absent.length + onLeaveAtt.length})</DialogTitle></DialogHeader>
          <DialogBody>
            {[...absent, ...onLeaveAtt].map((a, i) => (<div key={a._id} className="flex items-center gap-3 py-2">
                <div className={`w-9 h-9 rounded-xl ${COLORS[i % COLORS.length]} flex items-center justify-center text-white text-xs font-bold`}>{ini(empName(a))}</div>
      ...
      <Dialog open={modal === 'tasks'} onOpenChange={(o) => !o && setModal(null)}>
        <DialogContent className="max-w-2xl">
          <DialogHeader><DialogTitle>📋 {mgrDept} Active Tasks ({activeTasks.length})</DialogTitle></DialogHeader>
          <DialogBody>
            {activeTasks.map((t) => (<div key={t._id} className="py-2 border-b border-gray-50 dark:border-gray-700/50 last:border-0">
      ...
      <Dialog open={modal === 'leaves'} onOpenChange={(o) => !o && setModal(null)}>
        <DialogContent className="max-w-2xl">
          <DialogHeader><DialogTitle>🗓️ {mgrDept} Leave Requests ({pendingL.length} pending)</DialogTitle></DialogHeader>
          <DialogBody>
            {leaves.map((l, i) => (<div key={l.id} className="flex items-center justify-between py-2.5 border-b border-gray-50 dark:border-gray-700/50 last:border-0">
                <div className="flex items-center gap-3">
                ...
                {l.status === 'pending' ? (<div className="flex items-center gap-1.5 flex-shrink-0">
                  ...
                  </div>) : <Badge variant={l.status === 'approved' ? 'success' : 'destructive'} className="capitalize">{l.status}</Badge>}
              </div>))}
          </DialogBody>
        </DialogContent>
      </Dialog>
