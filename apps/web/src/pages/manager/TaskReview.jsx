...
const STATUS_LABELS = { pending: 'Pending', in_progress: 'In Progress', completed: 'Completed', overdue: 'Overdue', cancelled: 'Cancelled' };
const initials = (n) => n.split(' ').map((w) => w[0]).slice(0, 2).join('').toUpperCase();
const BLANK_FORM = { title: '', description: '', dept: '', assignedTo: [], fullTeam: false, priority: 'medium', due: '', estHours: '', link: '' };
...
export default function ManagerTaskReview() {
    ...
    const tasks = useMemo(() => apiTasks.map(mapApiTaskToRow).filter((t) => !effDept || t.dept === effDept), [apiTasks, effDept]);
    ...
    const counts = { '': cardBase.length };
    ['pending', 'in_progress', 'completed', 'overdue', 'cancelled'].forEach((s) => { counts[s] = cardBase.filter((t) => t.status === s).length; });
    const changeStatus = async (row, status) => {
        try {
            await updateTask({ id: row.id, status }).unwrap();
    ...
    const modalDeptEmployees = useMemo(() => apiEmployees.filter((e) => e.status === 'active' && (!form.dept || (typeof e.department === 'object' ? e.department?.name : e.department) === form.dept)), [apiEmployees, form.dept]);
    const toggleAssignee = (id) => setForm((p) => ({ ...p, assignedTo: p.assignedTo.includes(id) ? p.assignedTo.filter((x) => x !== id) : [...p.assignedTo, id], fullTeam: false }));
    ...
    const addTask = async () => {
        if (!form.title.trim() || form.assignedTo.length === 0 || !form.due) {
            toast.error('Fill required fields — title, at least one assignee, and due date');
            return;
        }
        setSubmitting(true);
        try {
            ...
            if (pendingFile) {
                try {
                    ...
                    toast.error('File upload failed — task will be created without the attachment');
                }
            }
            await Promise.all(form.assignedTo.map((empId) => createTask({
    ...
    const columns = [
        {
            ...
            cell: ({ row }) => (<div className="max-w-[200px]">
        ...
        { accessorKey: 'assignee', header: 'Assigned To', cell: ({ row }) => (<div className="flex items-center gap-2">
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl px-4 py-3">
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex flex-wrap gap-1">
            {['all', 'today', 'month', 'year'].map((p) => (<Button key={p} size="sm" variant={period === p ? 'default' : 'secondary'} onClick={() => setPeriod(p)} className="capitalize">
                {p === 'all' ? 'All' : p === 'today' ? 'Today' : p}
              </Button>))}
          </div>
          {period === 'month' && (<Select value={String(selMonth)} onValueChange={(v) => setSelMonth(Number(v))}>
              <SelectTrigger className="h-8 w-24 text-xs"><SelectValue /></SelectTrigger>
              <SelectContent>{MOS.map((m, i) => <SelectItem key={m} value={String(i)}>{m}</SelectItem>)}</SelectContent>
            </Select>)}
          {(period === 'month' || period === 'year') && (<Select value={String(selYear)} onValueChange={(v) => setSelYear(Number(v))}>
              <SelectTrigger className="h-8 w-24 text-xs"><SelectValue /></SelectTrigger>
          ...
          <Select value={statusF || 'all'} onValueChange={(v) => setStatusF(v === 'all' ? '' : v)}>
            ...
            <SelectContent><SelectItem value="all">All Status</SelectItem>{['pending', 'in_progress', 'completed', 'overdue', 'cancelled'].map((s) => <SelectItem key={s} value={s}>{STATUS_LABELS[s]}</SelectItem>)}</SelectContent>
          </Select>
        ...
        <DialogContent className="max-w-lg">
          ...
          <DialogBody>
            ...
            {!myDept && (<div>
                ...
                <Select value={form.dept || 'all'} onValueChange={(v) => setForm((p) => ({ ...p, dept: v === 'all' ? '' : v, assignedTo: [], fullTeam: false }))}>
            ...
            <div>
              ...
              <div className="border border-gray-200 dark:border-gray-600 rounded-xl max-h-40 overflow-y-auto divide-y divide-gray-50 dark:divide-gray-700">
                {modalDeptEmployees.length === 0 ? (<p className="text-xs text-gray-400 text-center py-4">No employees in this department</p>) : modalDeptEmployees.map((e) => (<label key={e._id} className="flex items-center gap-2 px-3 py-2 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/50">
              ...
              <DialogBody>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {['pending', 'in_progress', 'completed', 'cancelled'].map((s) => (<Button key={s} size="sm" variant={viewTask.status === s ? 'default' : 'outline'} onClick={() => { changeStatus(viewTask, s); setViewTask(null); }}>
