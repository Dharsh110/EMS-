...
export default function ManagerMyAttendance() {
    const { user } = useAuth();
    const designation = user?.designation || 'Manager';
    ...
    const records = (attData || []).filter((r) => (r.date?.slice(0, 10) || '') <= new Date().toISOString().slice(0, 10)).map((r) => {
        ...
        return {
            ...
            status: (r.status === 'present' ? 'Present' : r.status === 'absent' ? 'Absent' : r.status === 'leave' ? 'Absent' : r.status === 'half_day' ? 'Half Day' : r.status === 'holiday' ? 'Holiday' : r.status === 'weekend' ? 'Weekend' : 'Present'),
            isLate: !!r.isLate,
            notes: r.notes || '',
        };
    });
    const periodFiltered = records.filter((r) => {
        if (period === 'week')
            return r.date >= weekRange.from && r.date <= weekRange.to;
    ...
    return (<div className="space-y-4">
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-4 space-y-4">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {period === 'month' && (<>
              <Select value={String(selMonth)} onValueChange={(v) => setSelMonth(+v)}>
                <SelectTrigger className="w-28 h-8 text-xs"><SelectValue /></SelectTrigger>
                <SelectContent>{MOS.map((m, i) => <SelectItem key={m} value={String(i)}>{m}</SelectItem>)}</SelectContent>
              </Select>
              <Select value={String(selYear)} onValueChange={(v) => setSelYear(+v)}>
                <SelectTrigger className="w-24 h-8 text-xs"><SelectValue /></SelectTrigger>
                <SelectContent>{YEARS.map((y) => <SelectItem key={y} value={String(y)}>{y}</SelectItem>)}</SelectContent>
              </Select>
            </>)}
          {period === 'week' && <span className="text-[10px] text-gray-400">{weekRange.from} – {weekRange.to}</span>}
          {statusF && (<Button size="sm" variant="ghost" className="text-[11px] text-red-500 hover:bg-red-50" onClick={() => setStatusF('')}>Clear: {statusF} ✕</Button>)}
          <span className="text-xs text-gray-400 ml-auto">{filtered.length} records</span>
        </div>

        <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
          {statCards.map((s) => (<button key={s.label} onClick={() => setStatusF((p) => (p === s.statusKey ? '' : s.statusKey))} className={`rounded-xl p-3 text-center transition-all ${s.color} ${statusF === s.statusKey && s.statusKey ? 'ring-2 ring-blue-500 scale-105' : ''} hover:scale-105`}>
        ...
        <div className="grid grid-cols-2 gap-2">
          <button onClick={() => setStatusF((p) => (p === 'Late' ? '' : 'Late'))} className={`bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-800 rounded-xl p-2.5 flex items-center gap-3 hover:scale-[1.02] transition-all ${statusF === 'Late' ? 'ring-2 ring-blue-500' : ''}`}>
          ...
          <button onClick={() => setStatusF((p) => (p === 'Weekend' ? '' : 'Weekend'))} className={`bg-gray-50 dark:bg-gray-700/50 border border-gray-100 dark:border-gray-700 rounded-xl p-2.5 flex items-center gap-3 hover:scale-[1.02] transition-all ${statusF === 'Weekend' ? 'ring-2 ring-blue-500' : ''}`}>
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {filtered.length === 0 ? (<TableRow><TableCell colSpan={8} className="px-4 py-16 text-center">
                ...
                </TableCell></TableRow>) : filtered.map((r) => (<TableRow key={r.date} className={r.status === 'Weekend' ? 'opacity-50' : ''}>
