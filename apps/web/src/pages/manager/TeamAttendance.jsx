...
const deptOf = (e) => (e ? (typeof e.department === 'object' ? e.department?.name : e.department) || '' : '');
...
function aggregate(records) {
    ...
    records.forEach((r) => {
        ...
        const key = emp._id;
        if (!byEmp.has(key))
            byEmp.set(key, { name: `${emp.firstName} ${emp.lastName}`, code: emp.employeeCode || '', dept: deptOf(emp), present: 0, absent: 0, late: 0, halfDay: 0, leave: 0, totalHours: 0, officialMinutes: 0, count: 0 });
...
export default function ManagerTeamAttendance() {
    ...
    const empNames = useMemo(() => Array.from(new Set(apiEmployees.filter((e) => !effDept || deptOf(e) === effDept).map((e) => `${e.firstName} ${e.lastName}`))).sort((a, b) => a.localeCompare(b)), [apiEmployees, effDept]);
    ...
    const periodRange = useMemo(() => {
        ...
        const to = new Date(selYear, selMonth + 1, 0);
    ...
    const sortedToday = useMemo(() => {
        ...
        if (empF)
            data = data.filter((m) => m.name === empF);
    ...
    const SortBtn = ({ col, label }) => (<button onClick={() => { if (sortCol === col)
    ...
    return (<div className="space-y-4">
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        ...
        ].map(([key, label, val, bg, tc]) => (<button key={key} onClick={() => setStatusF(key)} className={`border rounded-2xl p-3 text-center hover:scale-105 transition-all ${bg} ${statusF === key ? 'ring-2 ring-blue-500' : ''}`}>
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl px-4 py-3">
        <div className="flex flex-wrap items-center gap-2">
          {['today', 'week', 'month', 'year'].map((p) => (<Button key={p} size="sm" variant={period === p ? 'default' : 'secondary'} onClick={() => setPeriod(p)} className="capitalize">
          ...
          {!myDept && (<Select value={deptF || 'all'} onValueChange={(v) => { setDeptF(v === 'all' ? '' : v); setEmpF(''); }}>
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
        ...
        <div className="overflow-x-auto">
          {isToday ? (<table className="w-full">
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-gray-700/50">
                {sortedToday.length === 0 ? (<tr><td colSpan={8} className="px-4 py-10 text-center text-sm text-gray-400">No records for this filter</td></tr>) : sortedToday.map((m, i) => (<tr key={m.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-gray-700/50">
                {monthlyLoading ? (<tr><td colSpan={8} className="px-4 py-10 text-center text-sm text-gray-400">Loading…</td></tr>) : sortedPeriod.length === 0 ? (<tr><td colSpan={8} className="px-4 py-10 text-center text-sm text-gray-400">No attendance data for this period</td></tr>) : sortedPeriod.map((m, i) => (<tr key={m.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
