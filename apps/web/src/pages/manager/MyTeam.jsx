...
import { mapApiEmployeeToRow } from '../admin/employees-columns';
...
export default function ManagerMyTeam() {
    ...
    const members = useMemo(() => apiEmployees.filter((e) => !myDept || (typeof e.department === 'object' ? e.department?.name : e.department) === myDept).map(mapApiEmployeeToRow), [apiEmployees, myDept]);
    ...
    const filtered = useMemo(() => members.filter((m) => (!statusF || m.status === statusF) &&
        (!deptF || m.dept === deptF) &&
        (!search || m.name.toLowerCase().includes(search.toLowerCase()) || m.code.toLowerCase().includes(search.toLowerCase()) || m.dept.toLowerCase().includes(search.toLowerCase()))), [members, statusF, deptF, search]);
    ...
    const openEdit = (m) => { setEditMember(m); setEditForm({ phone: m.phone, designation: m.designation, salary: m.salary }); };
    ...
    const attEntries = useMemo(() => monthlyRecords.filter((r) => (typeof r.employee === 'object' ? r.employee._id : r.employee) === viewMember?.id), [monthlyRecords, viewMember]);
    return (<div className="space-y-4">
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl px-4 py-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {['', 'Active', 'Inactive'].map((s) => (<Button key={s} size="sm" variant={statusF === s ? 'default' : 'secondary'} onClick={() => setStatusF(s)} className="capitalize">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {isLoading ? (<div className="col-span-3 py-16 text-center">
            <div className="inline-block w-6 h-6 border-2 border-blue-600 border-t-transparent rounded-full animate-spin mb-3"/>
            <p className="text-sm text-gray-400">Loading {myDept} team…</p>
          </div>) : filtered.length === 0 ? (<div className="col-span-3 py-16 text-center bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl">
          ...
          </div>) : filtered.map((m, i) => (<div key={m.id} className={'bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-4 hover:shadow-md transition-all ' + (m.status === 'Inactive' ? 'opacity-60' : '')}>
              ...
              <DialogBody>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {[['Employee ID', viewMember.code], ['Name', viewMember.name], ['Department', viewMember.dept], ['Designation', viewMember.designation], ['Email', viewMember.email], ['Phone', viewMember.phone], ['Joined', viewMember.joined], ['Salary', viewMember.salary ? `₹${Number(viewMember.salary).toLocaleString()}` : 'Not disclosed']].map(([k, v]) => (<div key={k} className="bg-gray-50 dark:bg-gray-700/30 rounded-xl p-2.5">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-xs">
                      ...
                      <tbody className="divide-y divide-gray-50 dark:divide-gray-700/50">
                        {attEntries.length === 0 ? (<tr><td colSpan={4} className="px-3 py-6 text-center text-xs text-gray-400">No records found</td></tr>) : attEntries.map((a) => (<tr key={a._id} className="hover:bg-gray-50 dark:hover:bg-gray-700/20">
