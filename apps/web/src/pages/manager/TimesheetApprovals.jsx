...
import { useGetAllTimesheetsQuery, useApproveTimesheetMutation, useRejectTimesheetMutation, } from '../../store/api/timesheetsApi';
...
export default function ManagerTimesheetApprovals() {
    ...
    const rows = useMemo(() => [...allTimesheets].filter((t) => !statusF || t.status === statusF).sort((a, b) => b.date.localeCompare(a.date)), [allTimesheets, statusF]);
    const counts = {
    ...
    const approve = async (t) => {
        try {
            await approveTimesheet(t._id).unwrap();
            toast.success(`${empName(t)}'s timesheet approved`);
        }
        catch (err) {
    ...
    const confirmReject = async () => {
        ...
        try {
            await rejectTimesheet({ id: rejectTarget._id, reason: rejectReason.trim() }).unwrap();
    ...
    const columns = [
        { accessorKey: 'employee', header: 'Employee', cell: ({ row }) => (<div><p className="text-xs font-semibold text-gray-800 dark:text-gray-200">{empName(row.original)}</p><p className="text-[10px] text-gray-400">{empCode(row.original)} · {empDept(row.original)}</p></div>), enableHiding: false },
    ...
    return (<div className="space-y-4">
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl px-4 py-3 flex flex-wrap items-center gap-2">
        ...
        {!mgrDept && (<Select value={deptF || 'all'} onValueChange={(v) => { setDeptF(v === 'all' ? '' : v); setEmpF(''); }}>
        ...
        <Select value={yearF || 'all'} onValueChange={(v) => { setYearF(v === 'all' ? '' : v); if (v !== 'all') {
        setFrom('');
        setTo('');
    } }}>
