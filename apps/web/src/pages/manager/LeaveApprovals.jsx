...
const LEAVE_TYPES = { 'Casual Leave': 'casual', 'Sick Leave': 'sick', 'Earned Leave': 'earned', 'Maternity Leave': 'maternity', 'Paternity Leave': 'paternity', 'Half Day': 'half_day', 'Unpaid Leave': 'unpaid' };
const STATUS_VARIANT = { pending: 'warning', approved: 'success', rejected: 'destructive', cancelled: 'gray' };
const diffDays = (from, to) => Math.max(1, Math.round((new Date(to).getTime() - new Date(from).getTime()) / 864e5) + 1);
export default function ManagerLeaveApprovals() {
    const { user } = useAuth();
    const mgrDept = user?.department || '';
    const [pageTab, setPageTab] = useState('approvals');
    ...
    const [myStatusF, setMyStatusF] = useState('');
    ...
    const confirmReject = async () => {
        ...
        try {
            await updateStatus({ id: rejectTarget.id, status: 'rejected', reason: rejectReason.trim() }).unwrap();
    ...
    const confirmCancel = async () => {
        if (!cancelTarget)
            return;
        try {
            await cancelLeave(cancelTarget.id).unwrap();
    ...
    const teamColumns = [
        { accessorKey: 'name', header: 'Employee', cell: ({ row }) => (<div><p className="text-xs font-semibold text-gray-800 dark:text-gray-200">{row.original.name}</p><p className="text-[10px] text-gray-400">{row.original.dept}</p></div>), enableHiding: false },
    ...
    const myColumns = [
        ...
        {
            id: 'actions', enableHiding: false,
            cell: ({ row }) => (<div className="flex gap-1 flex-wrap">
    ...
    const StatusCards = ({ c, active, onSelect }) => (<div className="grid grid-cols-2 sm:grid-cols-4 gap-3 flex-1">
      {[{ k: 'pending', l: 'Pending', c: 'bg-amber-50 dark:bg-amber-900/10 text-amber-600' }, { k: 'approved', l: 'Approved', c: 'bg-emerald-50 dark:bg-emerald-900/10 text-emerald-600' }, { k: 'rejected', l: 'Rejected', c: 'bg-red-50 dark:bg-red-900/10 text-red-500' }, { k: 'cancelled', l: 'Cancelled', c: 'bg-gray-50 dark:bg-gray-700/30 text-gray-500' }].map((s) => (<button key={s.k} onClick={() => onSelect(s.k)} className={`border border-gray-200 dark:border-gray-700 rounded-2xl p-3 text-left hover:scale-105 transition-all ${s.c} ${active === s.k ? 'ring-2 ring-blue-500' : ''}`}>
    ...
    return (<div className="space-y-4">
      ...
      {pageTab === 'approvals' && (<>
          ...
          <div className="flex items-center gap-2">
            ...
            {!mgrDept && (<Select value={deptF || 'all'} onValueChange={(v) => setDeptF(v === 'all' ? '' : v)}>
      ...
      {pageTab === 'my-leaves' && (<>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {myLeaveBalance.slice(0, 4).map((b) => {
                ...
                return (<div key={b.type} className={`border rounded-2xl p-3.5 ${b.bg}`}>
          ...
          <div className="flex items-center gap-2">
            <Select value={myStatusF || 'all'} onValueChange={(v) => setMyStatusF(v === 'all' ? '' : v)}>
              <SelectTrigger className="h-8 w-40 text-xs"><SelectValue placeholder="All Status"/></SelectTrigger>
              <SelectContent><SelectItem value="all">All Status</SelectItem>{['pending', 'approved', 'rejected', 'cancelled'].map((s) => <SelectItem key={s} value={s} className="capitalize">{s}</SelectItem>)}</SelectContent>
            </Select>
      ...
      {/* View leave details */}
      <Dialog open={!!viewL} onOpenChange={(o) => !o && setViewL(null)}>
        <DialogContent className="max-w-sm">
          <DialogHeader><DialogTitle>Leave Request</DialogTitle></DialogHeader>
          {viewL && (<>
              <DialogBody>
                {[['Employee', viewL.name], ['Leave Type', viewL.type], ['From', viewL.from], ['To', viewL.to], ['Days', `${viewL.days} day${viewL.days !== 1 ? 's' : ''}`], ['Applied On', viewL.applied], ['Status', viewL.status]].map(([k, v]) => (<div key={k} className="flex justify-between text-xs border-b border-gray-50 dark:border-gray-700/50 pb-2">
