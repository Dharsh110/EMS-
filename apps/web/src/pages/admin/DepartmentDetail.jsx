...
import { mapApiEmployeeToRow, buildEmployeeColumns } from './employees-columns';
...
export default function DepartmentDetail() {
    ...
    const deptEmployees = useMemo(() => apiEmployees.filter((e) => (typeof e.department === 'object' ? e.department?.name : e.department) === deptName).map(mapApiEmployeeToRow), [apiEmployees, deptName]);
    const q = search.toLowerCase();
    const filtered = deptEmployees.filter((e) => (!q || e.name.toLowerCase().includes(q) || e.code.toLowerCase().includes(q)) && (!statusF || e.status === statusF));
    ...
    const columns = useMemo(() => buildEmployeeColumns({
        onView: setViewing,
          ...
          {viewing && (<>
              <DialogBody>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[['Code', viewing.code], ['Email', viewing.email], ['Phone', viewing.phone || '—'], ['Type', viewing.type], ['Joined', viewing.joined], ['Salary', viewing.salary ? 'Rs.' + Number(viewing.salary).toLocaleString('en-IN') : '—']].map(([l, v]) => (<div key={l} className="bg-gray-50 dark:bg-gray-700/50 rounded-xl px-3 py-2">
