...
export function buildDepartmentColumns({ onViewMembers, onViewDetail, onEdit, onToggleStatus, onDelete }) {
    return [
        {
            accessorKey: 'name',
            header: 'Department',
        ...
        { accessorKey: 'location', header: 'Location', cell: ({ row }) => <span className="text-xs text-gray-500">{row.original.location || '—'}</span> },
        {
            ...
            cell: ({ row }) => {
                ...
                const label = h && typeof h === 'object' ? `${h.firstName} ${h.lastName}` : h || '—';
        ...
        },
        {
            ...
            header: 'Employees',
            cell: ({ row }) => (<Button variant="ghost" size="sm" className="h-auto py-0.5 text-[10px] font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/20 rounded-full hover:bg-indigo-100" onClick={() => onViewMembers(row.original)}>
