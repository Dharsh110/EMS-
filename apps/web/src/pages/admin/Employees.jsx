...
import { useGetEmployeesQuery, useCreateEmployeeMutation, useUpdateEmployeeMutation, useDeleteEmployeeMutation, } from '../../store/api/employeesApi';
import { DataTable } from '../../components/data-table/data-table';
import { buildEmployeeColumns, mapApiEmployeeToRow } from './employees-columns';
...
const BLANK = { empId: '', name: '', email: '', phone: '', dept: 'Engineering', designation: 'Software Engineer', type: 'Full-Time', status: 'Active', joined: '', salary: '', gender: 'Male', avatar: '', role: 'employee' };
const NO_DEPT = '__none__';
const ini = (n) => n.split(' ').map((x) => x[0]).join('').slice(0, 2).toUpperCase();
const typeToApi = { 'Full-Time': 'full_time', 'Part-Time': 'part_time', Contract: 'contract', Intern: 'intern' };
export default function AdminEmployees() {
    ...
    const handleChange = useCallback((field, value) => setForm((p) => ({ ...p, [field]: value })), []);
    ...
    const pickAvatar = (ev) => {
        ...
        r.onload = (e) => handleChange('avatar', e.target?.result);
    ...
    const filtered = emps.filter((e) => (!search || e.name.toLowerCase().includes(q) || e.email.toLowerCase().includes(q) || e.code.toLowerCase().includes(q)) &&
        (!deptF || e.dept === deptF) &&
        (!statusF || e.status === statusF));
    const dc = {};
    emps.forEach((e) => { dc[e.dept] = (dc[e.dept] || 0) + 1; });
    const columns = useMemo(() => buildEmployeeColumns({ onView: setViewing, onEdit: openEdit, onToggleStatus: toggleStatus, onDelete: setDeleteTarget }), []);
    return (<div className="space-y-4">
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
        {DEPTS.map((d) => (<button key={d} onClick={() => setDeptF(deptF === d ? '' : d)} className={'rounded-xl px-3 py-2.5 text-left border transition-all ' + (deptF === d ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 hover:border-blue-300')}>
          ...
          <DialogBody>
            ...
            <div className="grid grid-cols-2 gap-3">
              {modal === 'add' ? (<div>
                ...
                </div>) : (<div><Label>Employee ID</Label><Input className="mt-1" value={form.empId} disabled/></div>)}
          ...
          {viewing && (<>
              <DialogBody>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[['Emp ID', viewing.code], ['Email', viewing.email], ['Phone', viewing.phone || 'Not added'], ['Department', viewing.dept], ['Designation', viewing.designation], ['Type', viewing.type], ['Gender', viewing.gender], ['Joined', viewing.joined], ['Salary', viewing.salary ? 'Rs.' + Number(viewing.salary).toLocaleString('en-IN') + '/mo' : 'Confidential'], ['Status', viewing.status]].map(([l, v]) => (<div key={l} className="bg-gray-50 dark:bg-gray-700/50 rounded-xl px-3 py-2">
