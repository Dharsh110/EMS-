...
const DATE_KEY = { attendance: 'Date', leaves: 'Applied', tasks: 'Due', payroll: null, timesheets: 'Date', employees: 'JoinedDate', managers: 'JoinedDate', departments: 'Created' };
const SORT_KEY = { attendance: 'Employee', leaves: 'Employee', tasks: 'Task', payroll: 'Employee', timesheets: 'Employee', employees: 'Name', managers: 'Name', departments: 'DeptName' };
...
const EMP_KEY = { attendance: 'Employee', leaves: 'Employee', tasks: 'Assignee', payroll: 'Employee', timesheets: 'Employee', employees: null, managers: null, departments: null };
const REPORT_TYPES = [
...
export default function AdminReports() {
    ...
    const data = useMemo(() => {
        ...
        if (type === 'departments')
            return apiDepartments.map((d) => ({ DeptName: d.name, Head: typeof d.head === 'object' && d.head ? `${d.head.firstName} ${d.head.lastName}` : '—', ActiveEmployees: d.employeeCount ?? 0, Created: d.createdAt?.slice(0, 10) || '—' }));
        return [];
    ...
    const filterRows = (rows) => {
        let r = rows;
        if (deptF && type !== 'departments')
            r = r.filter((row) => row.Department === deptF);
        const ek = EMP_KEY[type];
        if (empF && ek)
            r = r.filter((row) => row[ek] === empF);
        if (period === 'all')
            return r;
        ...
        return r.filter((row) => {
            ...
            if (period === 'year')
                return new Date(d).getFullYear() === selYear;
            return true;
        });
    };
    const rows = useMemo(() => {
    ...
    return (<>
      <div className="space-y-4">
        <div className="flex flex-wrap gap-2">
          {REPORT_TYPES.map((r) => (<Button key={r.id} variant={type === r.id ? 'default' : 'outline'} size="sm" onClick={() => { setType(r.id); setPeriod('all'); setEmpF(''); }}>{r.icon} {r.label}</Button>))}
        </div>

        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl px-4 py-3">
          <div className="flex flex-wrap items-center gap-2">
            {['all', 'today', 'week', 'month', 'year'].map((p) => (<Button key={p} size="sm" variant={period === p ? 'default' : 'secondary'} onClick={() => setPeriod(p)}>{p === 'all' ? 'All Time' : p === 'week' ? 'This Week' : p}</Button>))}
            {period === 'month' && <Select value={String(selMonth)} onValueChange={(v) => setSelMonth(Number(v))}><SelectTrigger className="h-8 w-24 text-xs"><SelectValue /></SelectTrigger><SelectContent>{MOS.map((m, i) => <SelectItem key={m} value={String(i)}>{m}</SelectItem>)}</SelectContent></Select>}
            ...
            {type !== 'departments' && (<Select value={deptF || 'all'} onValueChange={(v) => setDeptF(v === 'all' ? '' : v)}>
            ...
            {EMP_KEY[type] && (<Select value={empF || 'all'} onValueChange={(v) => setEmpF(v === 'all' ? '' : v)}>
            ...
            <div className="flex items-center gap-1">
              ...
              {['asc', 'desc'].map((d) => <Button key={d} size="sm" variant={nameSort === d ? 'default' : 'secondary'} onClick={() => setNameSort((p) => (p === d ? '' : d))}>{d === 'asc' ? 'A→Z' : 'Z→A'}</Button>)}
            </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-gray-700/50">
                {loading ? (<tr><td colSpan={Math.max(headers.length, 1)} className="px-4 py-12 text-center"><p className="text-sm text-gray-400 animate-pulse">Loading {rt.label.toLowerCase()}…</p></td></tr>) : rows.length === 0 ? (<tr><td colSpan={Math.max(headers.length, 1)} className="px-4 py-12 text-center"><div className="flex flex-col items-center gap-2"><span className="text-3xl">{rt.icon}</span><p className="text-sm font-medium text-gray-500 dark:text-gray-400">No {rt.label.toLowerCase()} data found</p></div></td></tr>) : rows.map((r, i) => (<tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
      ...
      <Dialog open={!!deptDetail} onOpenChange={(o) => !o && setDeptDetail(null)}>
        <DialogContent className="max-w-2xl">
          ...
          <DialogBody>
            {deptEmps.length === 0 ? <p className="text-sm text-gray-400 text-center py-8">No employees found in {deptDetail}</p> : (<table className="w-full text-xs">
