...
const typeFromApi = { full_time: 'Full-Time', part_time: 'Part-Time', contract: 'Contract', intern: 'Intern' };
...
export function buildEmployeeColumns({ onView, onEdit, onToggleStatus, onDelete }) {
    return [
        {
            id: 'select',
        ...
        {
            accessorKey: 'code',
            header: ({ column }) => (<Button variant="ghost" size="sm" className="h-auto p-0 text-[10px] font-semibold uppercase tracking-wide text-gray-500 hover:bg-transparent" onClick={() => column.toggleSorting(column.getIsSorted() === 'asc')}>
        ...
        },
        {
            accessorKey: 'salary',
            header: 'Salary',
            cell: ({ row }) => (<span className="text-xs text-gray-500">
        ...
        },
        {
            accessorKey: 'status',
            header: 'Status',
            cell: ({ row }) => (<Badge variant={row.original.status === 'Active' ? 'success' : 'gray'}>{row.original.status}</Badge>),
        },
        {
            id: 'actions',
            enableHiding: false,
