...
import { mapApiTaskToRow, buildTaskColumns } from './tasks-columns';
...
const BLANK = { title: '', dept: '', assignedTo: [], fullTeam: false, priority: 'medium', due: '', estHours: '', description: '', link: '' };
export default function AdminTasks() {
    ...
    const SelBox = (props) => (<Select value={props.value || 'all'} onValueChange={(v) => props.onChange(v === 'all' ? '' : v)}>
    ...
    const goToAll = (status) => { setStatusF(status); setPeriod('all'); setPageView('all'); };
    const AssignDialog = (<Dialog open={modal} onOpenChange={(o) => { setModal(o); if (!o) {
        setForm(BLANK);
        setAssigneeSearch('');
        setPendingFile(null);
    } }}>
        ...
        <DialogBody>
          ...
          <div>
            ...
            <div className="border border-gray-200 dark:border-gray-600 rounded-xl max-h-40 overflow-y-auto divide-y divide-gray-50 dark:divide-gray-700">
              {modalDeptEmployees.length === 0 ? (<p className="text-xs text-gray-400 text-center py-4">No employees match</p>) : modalDeptEmployees.map((e) => (<label key={e._id} className="flex items-center gap-2 px-3 py-2 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/50">
    ...
    if (pageView === 'all')
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl px-4 py-3 flex flex-wrap items-center gap-2">
        <div className="flex flex-wrap gap-1">
          {[['all', 'All'], ['today', 'Today'], ['week', 'This Week'], ['month', 'Month'], ['year', 'Year']].map(([v, l]) => (<Button key={v} size="sm" variant={period === v ? 'default' : 'secondary'} onClick={() => setPeriod(v)}>{l}</Button>))}
        </div>
    ...
    return (<div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        ].map((s) => (<button key={s.l} onClick={s.action} className={`text-center bg-${s.c}-50 dark:bg-${s.c}-900/20 border border-${s.c}-100 dark:border-${s.c}-800 rounded-2xl p-3.5 hover:shadow-md transition-all`}>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50 dark:bg-gray-700/50">
              <tr>{['Department', 'Assigned', 'In Progress', 'Completed', 'Pending', 'Overdue'].map((h) => (<th key={h} className="px-4 py-2.5 text-left text-[10px] font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide whitespace-nowrap">{h}</th>))}</tr>
            </thead>
            ...
            return (<tr key={dept} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
      ...
      {remainingTasks.length > 0 && (<button onClick={() => setPageView('all')} className="text-xs text-purple-600 dark:text-purple-400 hover:underline font-medium">
