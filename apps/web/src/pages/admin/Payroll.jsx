...
import { mapApiPayrollToRow, buildPayrollColumns, fmt } from './payroll-columns';
...
export default function AdminPayroll() {
    ...
    const records = useMemo(() => apiPayroll.map((p) => {
        ...
        if (row.dept && deptIdToName.has(row.dept))
            row.dept = deptIdToName.get(row.dept);
        else if (!row.dept || !deptNames.includes(row.dept))
            row.dept = codeToDept.get(row.code) || '—';
        return row;
    ...
    const markPaid = async (row) => {
        try {
            await processPayment(row.id).unwrap();
    ...
    const runGenerate = async () => {
        if (pending.length === 0) {
            toast.error('All active employees already have payroll for this period');
            return;
        }
        setGenerating(true);
        try {
    ...
    return (<div className="space-y-4">
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl px-4 py-3">
        <div className="flex flex-wrap items-center gap-2">
          {['all', 'month', 'year'].map((p) => (<Button key={p} size="sm" variant={period === p ? 'default' : 'secondary'} onClick={() => setPeriod(p)} className="capitalize">{p}</Button>))}
        ...
        <DialogContent className="max-w-lg">
          ...
          <DialogBody>
            ...
            <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl px-4 py-3">
              ...
              {pending.length > 0 && (<div className="flex flex-wrap gap-1.5 mt-2">
                  {pending.map((e) => (<span key={e._id} className="text-[10px] bg-white dark:bg-gray-800 text-blue-700 dark:text-blue-400 px-2 py-0.5 rounded-full border border-blue-200 dark:border-blue-800">{e.firstName} {e.lastName}</span>))}
                </div>)}
            </div>
          </DialogBody>
          <DialogFooter>
