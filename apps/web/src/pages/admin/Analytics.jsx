...
export default function AdminAnalytics() {
    ...
    const employees = useMemo(() => apiEmployees.map((e) => ({
    ...
    const deptStats = useMemo(() => {
        ...
        employees.forEach((e) => {
            if (!e.dept)
                return;
            if (!deptMap[e.dept])
                deptMap[e.dept] = { dept: e.dept, count: 0, active: 0, inactive: 0, avgSalary: 0 };
            deptMap[e.dept].count++;
            if (e.status === 'active')
                deptMap[e.dept].active++;
            else
                deptMap[e.dept].inactive++;
            deptMap[e.dept].avgSalary += e.salary;
        });
        Object.values(deptMap).forEach((d) => { if (d.count > 0)
            d.avgSalary = Math.round(d.avgSalary / d.count); });
    ...
    const { taskStats, taskDetail } = useMemo(() => {
        ...
        apiTasks.forEach((t) => {
            ...
            if (!dept)
                return;
            if (!tMap[dept])
                tMap[dept] = { dept, total: 0, completed: 0, inProgress: 0, pending: 0 };
            tMap[dept].total++;
            if (t.status === 'completed')
                tMap[dept].completed++;
            else if (t.status === 'in_progress')
                tMap[dept].inProgress++;
            else
                tMap[dept].pending++;
            if (!tDetail[dept])
                tDetail[dept] = [];
    ...
    const leaveStats = useMemo(() => {
        const lMap = Object.fromEntries(ALL_LEAVE_TYPES.map((t) => [t, 0]));
    ...
    const modalEmps = useMemo(() => {
        ...
        if (modal === 'dept')
            return employees.filter((e) => e.dept === deptView);
        return [];
    }, [modal, deptView, employees]);
    const modalTitle = useMemo(() => {
        if (modal === 'total')
            return `All Employees (${totalEmp})`;
        if (modal === 'active')
            return `Active Employees (${activeEmp})`;
        if (modal === 'inactive')
            return `Inactive Employees (${inactiveEmp})`;
        if (modal === 'salary')
            return 'Salary Overview — All Employees';
        if (modal === 'dept')
            return `${deptView} Department`;
        return '';
    }, [modal, deptView, totalEmp, activeEmp, inactiveEmp]);
    const openDept = (dept) => { setDeptView(dept); setModal('dept'); };
    // ── EARNERS PAGE ──
    if (pageView === 'earners') {
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl px-4 py-3">
          <div className="flex flex-wrap items-center gap-2">
            <Select value={earnerDeptF || 'all'} onValueChange={(v) => setEarnerDeptF(v === 'all' ? '' : v)}>
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
          {filteredEarners.length === 0 ? (<div className="py-12 text-center"><span className="text-3xl">💰</span><p className="text-sm text-gray-400 mt-2">No employees yet</p></div>) : (<table className="w-full text-xs">
              <thead className="bg-gray-50 dark:bg-gray-700/50">
                <tr>{['Rank', 'Employee', 'Department', 'Role', 'Salary / Month', 'Status'].map((h) => (<th key={h} className="px-4 py-2.5 text-left text-[10px] font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide whitespace-nowrap">{h}</th>))}</tr>
              </thead>
              <tbody className="divide-y divide-gray-50 dark:divide-gray-700/50">
                {filteredEarners.map((e, i) => (<tr key={e.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30">
    ...
    if (pageView === 'task-detail') {
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
          {allTasks.length === 0 ? (<div className="py-12 text-center"><span className="text-3xl">📋</span><p className="text-sm text-gray-400 mt-2">No tasks</p></div>) : (<div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead className="bg-gray-50 dark:bg-gray-700/50">
                  <tr>{['Task', 'Assignee', 'Status', 'Priority', 'Due Date'].map((h) => (<th key={h} className="px-4 py-2.5 text-left font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide whitespace-nowrap text-[10px]">{h}</th>))}</tr>
                </thead>
                <tbody className="divide-y divide-gray-50 dark:divide-gray-700/50">
                  {allTasks.map((t, i) => (<tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-700/30">
    ...
    if (pageView === 'emp-detail') {
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
          {modalEmps.length === 0 ? (<div className="py-12 text-center"><span className="text-3xl">👥</span><p className="text-sm text-gray-400 mt-2">No employees</p></div>) : (<div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead className="bg-gray-50 dark:bg-gray-700/50">
                  <tr>{['ID', 'Name', 'Department', 'Role', 'Salary', 'Status', 'Joined'].map((h) => (<th key={h} className="px-4 py-2.5 text-left font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide whitespace-nowrap text-[10px]">{h}</th>))}</tr>
                </thead>
                <tbody className="divide-y divide-gray-50 dark:divide-gray-700/50">
                  {modalEmps.map((e) => (<tr key={e.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30">
    ...
    return (<div className="space-y-5">
      {/* Task Dept Modal */}
      <Dialog open={!!taskDept} onOpenChange={(o) => !o && setTaskDept('')}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>{taskDept} — Tasks ({(taskDetail[taskDept] || []).length})</DialogTitle>
          </DialogHeader>
          <DialogBody>
            {(taskDetail[taskDept] || []).length === 0 ? (<p className="text-sm text-gray-400 text-center py-8">No tasks for this department</p>) : (<table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-gray-100 dark:border-gray-700">
                    {['Task', 'Assignee', 'Status', 'Priority', 'Due Date'].map((h) => (<th key={h} className="pb-2 text-left font-semibold text-gray-500 dark:text-gray-400 pr-3">{h}</th>))}
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50 dark:divide-gray-700/50">
                  {(taskDetail[taskDept] || []).slice(0, 10).map((t, i) => (<tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-700/30">
          ...
          <DialogFooter>
            {(taskDetail[taskDept] || []).length > 10 && (<Button size="sm" className="text-xs" onClick={() => { setPageView('task-detail'); }}>View All →</Button>)}
      ...
      {loading ? (<div className="flex items-center justify-center h-48">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
                { label: 'Total Employees', value: totalEmp, sub: 'across all departments', color: 'blue', key: 'total' },
                { label: 'Active Employees', value: activeEmp, sub: totalEmp > 0 ? Math.round(activeEmp / totalEmp * 100) + '% of workforce' : '—', color: 'emerald', key: 'active' },
                { label: 'Inactive', value: inactiveEmp, sub: 'deactivated accounts', color: 'amber', key: 'inactive' },
                { label: 'Avg Salary', value: avgSalary > 0 ? 'Rs.' + avgSalary.toLocaleString('en-IN') : '—', sub: 'per month', color: 'purple', key: 'salary' },
            ].map((k) => (<div key={k.label} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-4 flex flex-col gap-1">
                ...
                {totalEmp > 0 && (<button onClick={() => setModal(k.key)} className={`mt-1 self-start text-[10px] px-2.5 py-0.5 rounded-lg font-semibold bg-${k.color}-50 dark:bg-${k.color}-900/20 text-${k.color}-600 dark:text-${k.color}-400 hover:opacity-80 transition-opacity`}>
                    View Details →
                  </button>)}
              </div>))}
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            ...
            ].map((s) => (<button key={s.label} onClick={() => navigate('/admin/tasks')} className={`text-left rounded-2xl p-4 flex items-center gap-3 hover:shadow-md transition-all ${s.color}`}>
          ...
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-5">
            ...
            {deptStats.length === 0 ? (<div className="py-8 text-center"><span className="text-2xl">👥</span><p className="text-sm text-gray-400 mt-2">No employees added yet</p></div>) : (<div className="space-y-3">
                {deptStats.map((d) => {
                    ...
                    return (<div key={d.dept}>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button onClick={() => openDept(d.dept)} className="text-[10px] px-2 py-0.5 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-lg hover:opacity-80 font-semibold transition-opacity">
                            View
                          </button>
                        </div>
                      </div>
          ...
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-5">
            ...
            {taskStats.length === 0 ? (<div className="py-8 text-center"><span className="text-2xl">📋</span><p className="text-sm text-gray-400 mt-2">No tasks assigned yet</p></div>) : (<div className="space-y-3">
                {taskStats.map((d) => {
                    ...
                    return (<div key={d.dept}>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button onClick={() => setTaskDept(d.dept)} className="text-[10px] px-2 py-0.5 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-lg hover:opacity-80 font-semibold transition-opacity">
                            View
                          </button>
                        </div>
                      </div>
          ...
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-5">
            ...
            {apiLeaves.length === 0 ? (<div className="py-8 text-center"><span className="text-2xl">🗓️</span><p className="text-sm text-gray-400 mt-2">No leaves applied yet</p></div>) : (<div className="space-y-2">
                {leaveStats.map((l) => {
          ...
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-5">
            <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
              ...
              {employees.length > 0 && (<Button variant="ghost" size="sm" className="text-xs text-purple-600 dark:text-purple-400" onClick={() => setPageView('earners')}>
                  View All Earners →
                </Button>)}
            </div>
            {employees.length === 0 ? (<div className="py-6 text-center"><span className="text-2xl">💰</span><p className="text-sm text-gray-400 mt-2">No employees yet</p></div>) : (<div className="space-y-2">
                {[...employees].sort((a, b) => b.salary - a.salary).slice(0, 5).map((e, i) => (<div key={e.id} className="flex items-center gap-3">
      ...
      <Dialog open={!!modal && pageView === 'main'} onOpenChange={(o) => !o && setModal(null)}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>{modalTitle}</DialogTitle>
          </DialogHeader>
          <DialogBody>
            {modalEmps.length === 0 ? (<p className="text-sm text-gray-400 text-center py-8">No employees</p>) : (<table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-gray-100 dark:border-gray-700">
                    {['ID', 'Name', 'Department', 'Role', 'Salary', 'Status', 'Joined'].map((h) => (<th key={h} className="pb-2 text-left font-semibold text-gray-500 dark:text-gray-400 pr-3">{h}</th>))}
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50 dark:divide-gray-700/50">
                  {modalEmps.slice(0, 10).map((e) => (<tr key={e.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30">
          ...
          <DialogFooter>
            {modalEmps.length > 10 && (<Button size="sm" className="text-xs" onClick={() => setPageView('emp-detail')}>View All ({modalEmps.length}) →</Button>)}
