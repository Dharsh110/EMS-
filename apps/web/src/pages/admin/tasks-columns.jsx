...
const PROGRESS_BY_STATUS = { pending: 0, in_progress: 50, completed: 100, overdue: 0, cancelled: 0 };
export function mapApiTaskToRow(t) {
...
export function buildTaskColumns({ onStatusChange }) {
    return [
        {
            accessorKey: 'title',
            header: 'Task',
          ...
          {row.original.submittedDescription && (<p className="text-[10px] text-blue-500 dark:text-blue-400 truncate mt-0.5" title={row.original.submittedDescription}>📝 {row.original.submittedDescription}</p>)}
          {!!row.original.submittedFiles?.length && (<p className="text-[10px] text-gray-400 mt-0.5" title={row.original.submittedFiles.map((f) => f.name).join(', ')}>📎 {row.original.submittedFiles.length} file{row.original.submittedFiles.length > 1 ? 's' : ''} attached</p>)}
        </div>),
            enableHiding: false,
        },
        ...
        { accessorKey: 'due', header: 'Due Date', cell: ({ row }) => <span className="text-xs text-gray-500 dark:text-gray-400 whitespace-nowrap">{row.original.due}</span> },
        {
            ...
            header: 'Progress',
            cell: ({ row }) => (<div className="flex items-center gap-1.5 w-24">
