...
const isToday = (from, to) => { const now = new Date().toISOString().slice(0, 10); return from <= now && now <= to; };
export default function AdminDashboard() {
    ...
    const depts = useMemo(() => [...new Set(apiEmployees.map((e) => (typeof e.department === 'object' ? e.department?.name : e.department) || '').filter(Boolean))], [apiEmployees]);
    ...
    return (<div className="space-y-5">
      <div className="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-2xl p-5 text-white">
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center gap-3">
            {[['Present Today', presentToday], ['Late Today', lateToday], ['On Leave', onLeave]].map(([l, v]) => (<div key={l} className="bg-white/20 backdrop-blur-sm rounded-xl px-4 py-2 text-center">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        ].map((c) => (<button key={c.label} onClick={() => navigate(c.path)} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-4 text-left hover:shadow-md hover:-translate-y-0.5 transition-all">
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-700 flex-wrap gap-2">
          ...
          {depts.length > 0 && (<Button size="sm" onClick={() => setShowMonthlyDetail(true)}>View Dept Breakdown →</Button>)}
        </div>
        <div className="p-5">
          ...
          <div className="overflow-x-auto">
            ...
            return (<tr key={`${m.month}-${m.year}`} className="hover:bg-gray-50 dark:hover:bg-gray-700/30">
          ...
          {trend.every((m) => m.present + m.absent === 0) && (<p className="text-[11px] text-gray-400 mt-3 text-center">No check-ins recorded yet — data will populate once employees check in.</p>)}
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-700">
          ...
          <div className="flex items-center gap-2">
            {depts.length > 0 && (<Select value={attDept || 'all'} onValueChange={(v) => setAttDept(v === 'all' ? '' : v)}>
        ...
        <div className="overflow-x-auto">
          {histFiltered.length === 0 ? (<div className="py-10 text-center"><span className="text-3xl">👥</span><p className="text-sm text-gray-400 mt-2">No employees yet</p></div>) : (<table className="w-full">
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-gray-700/50">
                ...
                return (<tr key={e.code} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
        ...
        {taskRows.length === 0 ? (<div className="py-10 text-center"><span className="text-3xl">📋</span><p className="text-sm text-gray-400 mt-2">No tasks yet</p></div>) : (<div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-gray-700/50">
                {taskRows.map((t) => (<tr key={t.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-5">
        ...
        {activity.length === 0 ? (<div className="py-6 text-center"><span className="text-2xl">🕐</span><p className="text-sm text-gray-400 mt-2">No recent activity</p></div>) : (<div className="space-y-3">
            {activity.map((a) => (<div key={a.id} className="flex items-center gap-3 py-2 border-b border-gray-50 dark:border-gray-700/50 last:border-0">
        ...
        <DialogContent className="max-w-3xl">
          ...
          <DialogBody>
            <div className="flex items-center gap-2 flex-wrap mb-3">
              ...
              <Select value={mdDeptSort || 'name'} onValueChange={(v) => setMdDeptSort(v === 'name' ? '' : v)}>
            ...
            {deptAttData.length === 0 ? (<p className="text-sm text-gray-400 text-center py-8">No department data available</p>) : (<table className="w-full">
                ...
                if (mdDeptSort === 'present_desc')
                    rows.sort((a, b) => b.present - a.present);
                else if (mdDeptSort === 'att_desc')
                    rows.sort((a, b) => b.pct - a.pct);
                else if (mdDeptSort === 'att_asc')
                    rows.sort((a, b) => a.pct - b.pct);
                else
                    rows.sort((a, b) => a.dept.localeCompare(b.dept));
                return rows.map((r) => (<tr key={r.dept} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
