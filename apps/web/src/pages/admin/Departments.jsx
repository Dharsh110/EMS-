...
import { useGetDepartmentsQuery, useCreateDepartmentMutation, useUpdateDepartmentMutation, useDeleteDepartmentMutation, } from '../../store/api/departmentsApi';
...
const BLANK = { name: '', code: '', location: 'HQ', description: '', isActive: true, head: '', headEmail: '' };
export default function AdminDepartments() {
    ...
    const openEdit = (d) => {
        const headId = typeof d.head === 'object' && d.head ? d.head._id : d.head || '';
    ...
    const headOptions = useMemo(() => [...apiEmployees]
    ...
    const suggestedHeadEmail = useMemo(() => {
        if (!selectedHeadEmployee || !form.name.trim())
            return '';
    ...
    const save = async () => {
        ...
        try {
            ...
            if (result.credentials)
                setNewCreds({ ...result.credentials, deptName: form.name });
            if (result.headError)
                toast.error(`Department saved, but head assignment failed: ${result.headError}`);
        }
        catch (err) {
    ...
    const columns = useMemo(() => buildDepartmentColumns({
    ...
    return (<div className="space-y-4">
      <div className="grid grid-cols-3 gap-3">
        ...
        ].map((s) => (<div key={s.l} className={`bg-gradient-to-br ${s.c} rounded-2xl p-4 text-white`}>
        ...
        <DialogContent className="max-w-sm">
          ...
          <DialogBody>
            ...
            <div>
              ...
              <SearchableSelect className="mt-1" options={headOptions} value={form.head} onChange={(v) => setForm((p) => ({ ...p, head: v, headEmail: v ? p.headEmail : '' }))} placeholder="Search by name or email…" emptyOptionLabel="— None —"/>
            </div>
            {form.head && selectedHeadEmployee && selectedHeadEmployee.role !== 'manager' && (<div>
                <Label>New Login Email <span className="text-gray-400 font-normal">(optional — promotes to Manager)</span></Label>
                <div className="mt-1 flex gap-1.5">
                  <Input className="flex-1" value={form.headEmail} onChange={(e) => setForm((p) => ({ ...p, headEmail: e.target.value }))} placeholder={suggestedHeadEmail || 'e.g. name.department.manager@zetaq.com'}/>
                  {suggestedHeadEmail && (<Button type="button" size="sm" variant="outline" className="flex-shrink-0" onClick={() => setForm((p) => ({ ...p, headEmail: suggestedHeadEmail }))}>
                      Use
                    </Button>)}
                </div>
      ...
      {/* Members dialog */}
      <Dialog open={!!membersDept} onOpenChange={(o) => !o && setMembersDept(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>👥 {membersDept?.name} — Members</DialogTitle>
          </DialogHeader>
          <DialogBody>
            {deptMembers.length === 0 ? (<p className="text-sm text-gray-400 text-center py-8">No employees in this department yet.</p>) : (<div className="space-y-2">
                {deptMembers.map((m) => (<div key={m.id} className="flex items-center justify-between bg-gray-50 dark:bg-gray-700/50 rounded-xl px-3 py-2">
                    <div>
