...
import { useGetAllTimesheetsQuery, useGetTimesheetSummaryQuery } from '../../store/api/timesheetsApi';
...
const STATUS_VARIANT = { draft: 'gray', pending_approval: 'warning', approved: 'success', rejected: 'destructive' };
const fmtHours = (m) => (m / 60).toFixed(2) + 'h';
const empName = (t) => (typeof t.employee === 'object' ? `${t.employee.firstName} ${t.employee.lastName}` : 'Employee');
const empCode = (t) => (typeof t.employee === 'object' ? t.employee.employeeCode || '' : '');
const empDept = (t) => (typeof t.employee === 'object' && t.employee.department && typeof t.employee.department === 'object' ? t.employee.department.name : '');
...
const deptOf = (e) => (typeof e.department === 'object' ? e.department?.name : e.department) || '';
export default function AdminTimesheets() {
    ...
    const cards = [
        { label: 'Approved Hours', value: (summary?.approvedHours ?? 0).toFixed(2) + 'h', sub: `${summary?.approvedCount ?? 0} timesheets`, cls: 'bg-emerald-50 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-800 text-emerald-600', status: 'approved' },
        { label: 'Pending Hours', value: (summary?.pendingHours ?? 0).toFixed(2) + 'h', sub: `${summary?.pendingCount ?? 0} timesheets`, cls: 'bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800 text-amber-600', status: 'pending_approval' },
        { label: 'Rejected Hours', value: (summary?.rejectedHours ?? 0).toFixed(2) + 'h', sub: `${summary?.rejectedCount ?? 0} timesheets`, cls: 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800 text-red-500', status: 'rejected' },
        { label: 'Total Timesheets', value: String(summary?.totalCount ?? 0), sub: 'in selected range', cls: 'bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800 text-blue-600', status: '' },
    ];
    return (<div className="space-y-4">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {cards.map((c) => (<button key={c.label} onClick={() => setStatusF(c.status)} className={`text-left border rounded-2xl p-4 hover:shadow-md transition-all ${c.cls} ${statusF === c.status ? 'ring-2 ring-current' : ''}`}>
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl px-4 py-3 flex flex-wrap items-end gap-2">
        ...
        <div>
          ...
          <Select value={yearF || 'all'} onValueChange={(v) => setYearF(v === 'all' ? '' : v)}>
        ...
        <div>
          ...
          <Select value={deptF || 'all'} onValueChange={(v) => { setDeptF(v === 'all' ? '' : v); setEmpF(''); }}>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-gray-700/50">
              {isLoading ? (<tr><td colSpan={6} className="px-4 py-10 text-center text-sm text-gray-400">Loading…</td></tr>) : rows.length === 0 ? (<tr><td colSpan={6} className="px-4 py-10 text-center text-sm text-gray-400">No timesheets found for this filter</td></tr>) : rows.map((t) => (<tr key={t._id} onClick={() => setViewT(t)} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors cursor-pointer">
        ...
        <DialogContent className="max-w-lg">
          ...
          {viewT && (<DialogBody>
              <div className="space-y-2">
                {viewT.entries.map((e, i) => (<div key={i} className="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-3 space-y-1.5">
              ...
              {viewT.rejectionReason && (<div className="mt-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-3">
              ...
              <div className="mt-3">
                ...
                <div className="space-y-1">
                  {viewT.auditTrail.map((a, i) => (<p key={i} className="text-[10px] text-gray-400">
