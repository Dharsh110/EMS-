...
import { mapApiAttendanceToRow, attendanceColumns } from './attendance-columns';
...
export default function AdminAttendance() {
    const [attTab, setAttTab] = useState('employee');
    ...
    const empNames = useMemo(() => Array.from(new Set(apiEmployees
        .filter((e) => (attTab === 'manager' ? e.role === 'manager' : e.role !== 'manager'))
        .filter((e) => !deptF || deptOf(e) === deptF)
        .map((e) => `${e.firstName} ${e.lastName}`)))
        .sort((a, b) => a.localeCompare(b)), [apiEmployees, deptF, attTab]);
