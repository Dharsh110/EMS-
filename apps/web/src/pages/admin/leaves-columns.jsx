...
export function mapApiLeaveToRow(l) {
...
export function buildLeaveColumns({ onApprove, onReject, onRevoke }) {
    return [
        {
            accessorKey: 'name',
            header: 'Employee',
        ...
        { accessorKey: 'status', header: 'Status', cell: ({ row }) => <Badge variant={STATUS_VARIANT[row.original.status]} className="capitalize">{row.original.status}</Badge> },
        {
            ...
            <Button size="icon" variant="ghost" className="h-6 w-6 bg-red-50 text-red-500 hover:bg-red-100" title="Reject" onClick={() => onReject(l)}><X className="h-3.5 w-3.5"/></Button>
                ...
                if (l.status === 'approved')
                    return <Button size="sm" variant="ghost" className="h-auto py-0.5 text-[10px] bg-red-50 text-red-500 hover:bg-red-100" onClick={() => onRevoke(l)}>Revoke</Button>;
                if (l.status === 'rejected')
                    return <Button size="sm" variant="ghost" className="h-auto py-0.5 text-[10px] bg-emerald-50 text-emerald-600 hover:bg-emerald-100" onClick={() => onApprove(l)}>Re-approve</Button>;
