...
export default function AdminSettings() {
    ...
    const [pcdAt, setPcdAt] = useState(null);
    ...
    useEffect(() => {
        ...
        if (prefs.company)
            setCompany((p) => ({ ...p, ...prefs.company }));
        if (prefs.notifications)
            setNotifs((p) => ({ ...p, ...prefs.notifications }));
        if (prefs.general)
            setGeneral((p) => ({ ...p, ...prefs.general }));
    }, [apiProfile]);
    const saveGen = async () => {
        try {
    ...
    const saveCo = async () => {
        try {
            await updateMyProfile({ preferences: { company } }).unwrap();
    ...
    const Toggle = ({ label, k, desc }) => (<div className="flex items-center justify-between py-3 border-b border-gray-50 dark:border-gray-700 last:border-0">
      ...
      <Switch checked={notifs[k]} onCheckedChange={(v) => setNotifs((p) => ({ ...p, [k]: v }))}/>
    </div>);
      ...
      <Tabs defaultValue="general">
        ...
        <TabsContent value="general">
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-5 space-y-4">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div><Label>Timezone</Label><Select value={general.timezone} onValueChange={(v) => setGeneral((p) => ({ ...p, timezone: v }))}><SelectTrigger className="mt-1"><SelectValue /></SelectTrigger><SelectContent>{['Asia/Kolkata', 'UTC', 'America/New_York'].map((o) => <SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent></Select></div>
              <div><Label>Language</Label><Select value={general.language} onValueChange={(v) => setGeneral((p) => ({ ...p, language: v }))}><SelectTrigger className="mt-1"><SelectValue /></SelectTrigger><SelectContent>{['English', 'Tamil', 'Hindi'].map((o) => <SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent></Select></div>
              <div><Label>Date Format</Label><Select value={general.dateFormat} onValueChange={(v) => setGeneral((p) => ({ ...p, dateFormat: v }))}><SelectTrigger className="mt-1"><SelectValue /></SelectTrigger><SelectContent>{['DD/MM/YYYY', 'MM/DD/YYYY', 'YYYY-MM-DD'].map((o) => <SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent></Select></div>
            </div>
        ...
        <TabsContent value="company">
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div><Label>Currency</Label><Select value={company.currency} onValueChange={(v) => setCompany((p) => ({ ...p, currency: v }))}><SelectTrigger className="mt-1"><SelectValue /></SelectTrigger><SelectContent>{['INR', 'USD', 'EUR', 'GBP'].map((o) => <SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent></Select></div>
              <div><Label>Fiscal Year Starts</Label><Select value={company.fiscalYear} onValueChange={(v) => setCompany((p) => ({ ...p, fiscalYear: v }))}><SelectTrigger className="mt-1"><SelectValue /></SelectTrigger><SelectContent>{['January', 'April', 'July', 'October'].map((o) => <SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent></Select></div>
            </div>
        ...
        <TabsContent value="security">
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-5 space-y-4">
            ...
            {[['Current Password', 'current', 'c'], ['New Password', 'next', 'n'], ['Confirm', 'confirm', 'cf']].map(([l, k, sk]) => (<div key={k}>
                <Label>{l}</Label>
                ...
                {k === 'next' && pwd.next && (<div className="mt-2 space-y-1">
        ...
        <TabsContent value="help">
          <div className="space-y-4">
        ...
        ].map((c) => (<div key={c.label} className={`border rounded-2xl p-4 ${c.color}`}>
        ...
        ].map((f, i) => (<details key={i} className="group">
            ...
            <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-5">
              ...
              {ticketSent ? (<div className="text-center py-6">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div><Label>Category</Label><Select value={ticket.category} onValueChange={(v) => setTicket((p) => ({ ...p, category: v }))}><SelectTrigger className="mt-1"><SelectValue /></SelectTrigger><SelectContent>{['General', 'Technical', 'Billing', 'Feature Request', 'Bug Report'].map((c) => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent></Select></div>
                  </div>
