...
const ROLES = [
    { id: 'admin', label: 'Admin', desc: 'Full system access', ab: 'A',
    ...
    { id: 'manager', label: 'Manager', desc: 'Team management', ab: 'M',
    ...
    { id: 'employee', label: 'Employee', desc: 'Personal workspace', ab: 'E',
...
export default function Login() {
    ...
    // };
    return (<div className="min-h-screen flex bg-gray-50 dark:bg-gray-950 overflow-hidden relative transition-colors duration-300">
      {/* Animated blobs */}
      ...
      {/* Theme toggle */}
      <button onClick={toggleTheme} className="absolute top-4 right-4 z-20 flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-white/[0.06] text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-white/10 text-xs font-medium transition-all">
        {isDark
      ...
      <div className="hidden lg:flex flex-col justify-between w-[400px] flex-shrink-0 relative z-10 p-10 border-r border-gray-200 dark:border-white/[0.07] bg-white dark:bg-transparent">
        ...
        <div className="space-y-5">
        ...
        ].map(f => (<div key={f.label} className="flex items-center gap-3">
          ...
          <div className="flex gap-2 flex-wrap">
            {ROLES.map(r => (<span key={r.id} className={`text-xs px-3 py-1 rounded-full font-medium ${r.lbg} ${r.ltext} dark:bg-white/[0.07] dark:text-gray-400`}>{r.label}</span>))}
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 sm:px-8 relative z-10 py-12">
        ...
        <div className="w-full max-w-md">
          ...
          <div className="grid grid-cols-3 gap-2 mb-6">
            {ROLES.map(r => (<button key={r.id} type="button" onClick={() => setRole(r.id)} className={`relative flex flex-col items-center gap-1.5 py-3 px-2 rounded-xl border-2 transition-all duration-200 ${role === r.id ? `${r.selBorder} ${r.selBg} shadow-sm` : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600 bg-white dark:bg-gray-800/50'}`}>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text" placeholder="you@company.com or EMP031" autoComplete="username" className={`w-full pl-10 pr-4 py-2.5 rounded-xl border text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 transition-all ${errors.email ? 'border-red-400 focus:ring-red-400/30' : 'border-gray-200 dark:border-gray-600 focus:border-blue-400 focus:ring-blue-400/20'}`} {...register('email', { required: 'Email or Employee ID required' })}/>
                </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type={showPwd ? 'text' : 'password'} placeholder="Your password" autoComplete="current-password" className={`w-full pl-10 pr-14 py-2.5 rounded-xl border text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 transition-all ${errors.password ? 'border-red-400 focus:ring-red-400/30' : 'border-gray-200 dark:border-gray-600 focus:border-blue-400 focus:ring-blue-400/20'}`} {...register('password', { required: 'Password required', minLength: { value: 6, message: 'Min 6 characters' } })}/>
              ...
              <button type="submit" disabled={loading} className={`w-full py-2.5 rounded-xl text-sm font-bold text-white shadow-md transition-all hover:shadow-lg hover:opacity-90 active:scale-[0.99] disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2 mt-1 ${sel.bg}`}>
        ...
        */}
          ...
          <div className="mt-4 border border-dashed border-gray-300 dark:border-gray-700 rounded-xl overflow-hidden">
            <button onClick={() => setShowDemo(!showDemo)} className="w-full flex items-center justify-between px-4 py-2.5 text-xs font-medium text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
            ...
            {showDemo && (<div className="bg-gray-50 dark:bg-gray-800/40">
                {/* Admin & Employee */}
                <div className="divide-y divide-gray-100 dark:divide-gray-700/50 max-h-80 overflow-y-auto">
                  {DEMO_CREDS.map(c => (<button key={c.email} onClick={() => fillDemo(c)} className="w-full flex items-center justify-between px-4 py-2.5 hover:bg-gray-100 dark:hover:bg-gray-700/50 transition-colors text-left group">
                      <div>
