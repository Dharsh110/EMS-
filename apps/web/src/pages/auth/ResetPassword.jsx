...
const checks = [
    ...
    { key: 'num', label: 'Number', test: (v) => /[0-9]/.test(v) },
    { key: 'sym', label: 'Symbol', test: (v) => /[^A-Za-z0-9]/.test(v) },
];
const ResetPassword = () => {
    ...
    if (done)
        return (<div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-950 px-4">
      ...
      <div className="w-full max-w-sm animation-fade-in">
        ...
        <div className="card p-6">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div>
              <Input label="New password" type={showPwd ? 'text' : 'password'} placeholder="Min. 8 characters" error={errors.password?.message} rightElement={<button type="button" onClick={() => setShowPwd(!showPwd)} className="text-gray-400 hover:text-gray-600"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M15 12a3 3 0 11-6 0 3 3 0 016 0zM2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg></button>} {...register('password', { required: 'Password is required', minLength: { value: 8, message: 'Min 8 characters' } })}/>
              {pwd && (<div className="mt-2">
                  <div className="flex gap-1 mb-1.5">
                    {[1, 2, 3, 4].map((i) => (<div key={i} className={clsx('flex-1 h-1 rounded-full transition-colors', i <= score ? strengthColors[score] : 'bg-gray-200 dark:bg-gray-600')}/>))}
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {checks.map((c) => (<span key={c.key} className={clsx('text-xs px-2 py-0.5 rounded-full', c.test(pwd) ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' : 'bg-gray-100 dark:bg-gray-700 text-gray-400')}>
                        {c.test(pwd) ? '✓ ' : ''}{c.label}
                      </span>))}
            ...
            <Input label="Confirm new password" type={showPwd ? 'text' : 'password'} placeholder="Re-enter password" error={errors.confirm?.message} {...register('confirm', {
