...
const TYPE_ICON = { leave: '🗓️', attendance: '⏰', employee: '👤', task: '📋', payroll: '💰', report: '📊', calendar: '📅', system: '⚙️', reminder: '🔔' };
const TYPE_CLR = {
...
const inPeriod = (d, period, selM, selY) => {
    const n = new Date();
    if (period === 'today') {
    ...
    if (period === 'week')
        return d >= new Date(n.getTime() - 7 * 864e5);
    if (period === 'month')
...
export default function Notifications() {
    ...
    const DEPARTMENTS = useMemo(() => ['all', ...apiDepartments.map((d) => d.name)], [apiDepartments]);
    ...
    const doDelete = () => {
        ...
        if (target)
            setDeleted(p => [target, ...p.filter(n => n.id !== deleteId)]);
    ...
    const displayList = applyFilters(section === 'active' ? activeNotifs :
        section === 'archived' ? archivedNotifs : deleted);
    const unreadCount = activeNotifs.length;
    const FilterBar = () => (<div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-3">
      <div className="flex flex-wrap items-center gap-2">
        {['all', 'today', 'week', 'month', 'year'].map(p => (<button key={p} onClick={() => setPeriod(p)} className={`text-[11px] px-2.5 py-1.5 rounded-lg font-semibold whitespace-nowrap transition-all ${period === p ? 'bg-blue-600 text-white shadow' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-600'}`}>
        ...
        <div className="ml-auto flex items-center gap-2 flex-wrap">
          {role !== 'employee' && (<select value={deptF} onChange={e => setDeptF(e.target.value)} className={SEL}>
          ...
          {section === 'active' && unreadCount > 0 && (<button onClick={markAllRead} className="text-xs text-blue-600 dark:text-blue-400 hover:underline font-medium whitespace-nowrap">Mark all read</button>)}
        </div>
      </div>
    </div>);
    const NotifRow = ({ n, showReadBtn }) => (<div className={`flex items-start gap-4 px-5 py-4 transition-colors ${n.unread ? 'bg-blue-50/40 dark:bg-blue-900/5 hover:bg-blue-50/60 dark:hover:bg-blue-900/10' : 'hover:bg-gray-50 dark:hover:bg-gray-700/30'}`}>
      ...
      <div className="flex items-center gap-1 flex-shrink-0">
        {showReadBtn && n.unread && (<button onClick={() => markRead(n.id)} className="text-[10px] px-2 py-1 rounded-lg bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 hover:bg-blue-100 font-semibold transition-colors whitespace-nowrap">
            Mark Read
          </button>)}
        <button onClick={() => confirmDelete(n.id)} className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 text-gray-400 hover:text-red-500 transition-colors" title="Delete">
    ...
    return (<div className="space-y-4 max-w-3xl">
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-2 flex gap-1">
        ...
        ].map(([s, l, cnt, tc]) => (<button key={s} onClick={() => setSection(s)} className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-semibold transition-all ${section === s ? 'bg-blue-600 text-white shadow' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700'}`}>
            {l}
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
        {displayList.length === 0 ? (<div className="px-4 py-16 text-center">
            ...
            {displayList.map(n => (<NotifRow key={n.id} n={n} showReadBtn={section === 'active'}/>))}
          </div>)}
      </div>

      {/* Delete confirm modal */}
      {deleteId && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
