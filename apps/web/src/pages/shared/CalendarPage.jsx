...
const ET_ICON = { holiday: '🏖️', meeting: '📅', reminder: '🔔', deadline: '⏰', company_event: '🎉', birthday: '🎂' };
const initForm = { title: '', type: 'meeting', date: '', endDate: '', visibleTo: 'all', description: '' };
function mapEvent(e) {
...
export default function CalendarPage() {
    ...
    const dayEvents = (day) => events.filter((e) => { const d = new Date(e.date); return d.getFullYear() === calYear && d.getMonth() === calMonth && d.getDate() === day; });
    ...
    return (<div className="space-y-5">
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
        ...
        <div className="p-4">
          ...
          <div className="grid grid-cols-7 gap-1">
            ...
            return (<div key={day} onClick={() => openAdd(day)} className={`h-16 sm:h-20 rounded-xl p-1.5 cursor-pointer transition-all hover:bg-gray-50 dark:hover:bg-gray-700 border ${isTd ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-100 dark:border-gray-700'}`}>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-gray-700/50">
              {monthEvents.length === 0 ? (<tr><td colSpan={6} className="px-4 py-12 text-center"><div className="flex flex-col items-center gap-2"><span className="text-3xl">📅</span><p className="text-sm font-medium text-gray-500 dark:text-gray-400">No events in {MONTHS_FULL[calMonth]} {calYear}</p><p className="text-xs text-gray-400">Click on a day in the calendar or use "Add Event" to create one</p></div></td></tr>) : monthEvents.map((e) => (<tr key={e.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Select value={form.type} onValueChange={(v) => setForm((p) => ({ ...p, type: v }))}>
                  ...
                  <SelectContent>{['meeting', 'holiday', 'reminder', 'deadline', 'company_event', 'birthday'].map((t) => <SelectItem key={t} value={t} className="capitalize">{t.replace('_', ' ')}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <div>
                <Label>Visible To</Label>
                <Select value={form.visibleTo} onValueChange={(v) => setForm((p) => ({ ...p, visibleTo: v }))}>
                  <SelectTrigger className="mt-1"><SelectValue /></SelectTrigger>
      ...
      <Dialog open={!!viewEvt} onOpenChange={(o) => !o && setViewEvt(null)}>
